...
import { MAX_GROUP_MEMBERS, inviteParamsSchema, type InviteJoinResult, type InvitePreview } from '@enbox/shared';
...
type Target = { kind: 'chat'; chat: ChatRow; avatarKey: string | null } | { kind: 'community'; community: CommunityRow; avatarKey: string | null };

async function findInvite(dbx: DbOrTx, code: string): Promise<Target> {
  const [c] = await dbx.select({ chat: chats, avatarKey: media.storageKey }).from(chats).leftJoin(media, eq(media.id, chats.avatarMediaId)).where(eq(chats.inviteCode, code)).limit(1);
  if (c) {
    if (c.chat.type === 'channel' || (c.chat.type === 'group' && !c.chat.isAnnouncement)) return { kind: 'chat', chat: c.chat, avatarKey: c.avatarKey };
...
async function preview(dbx: DbOrTx, me: string, code: string): Promise<InvitePreview> {
  ...
  if (target.kind === 'community') {
    ...
    return { code, kind: 'community', id: c.id, name: c.name, description: c.description, avatarUrl, memberCount, communityId: null, communityName: null, isMember, canJoin: !isMember && !reason, reason };
  }
  ...
  const base = { code, id: chat.id, name: chat.name ?? '', description: chat.description, avatarUrl, memberCount, isMember };
  if (chat.type === 'channel') {
    return { ...base, kind: 'channel', communityId: null, communityName: null, canJoin: !isMember, reason: null };
  }
  ...
  if (!isMember) {
    ...
    else if (community && !(await getCommunityMember(dbx, community.id, me)) && (await communityMemberCount(dbx, community.id)) >= MAX_GROUP_MEMBERS) {
      reason = 'This community is full';
    }
  }
  return { ...base, kind: 'group', communityId: community?.id ?? null, communityName: community?.name ?? null, canJoin: !isMember && !reason, reason };
}
...
router.post('/invites/:code/join', inviteLimiter, async (req, res) => {
  const me = authUserId(req);
  const { code } = parse(inviteParamsSchema, req.params);
  const joined = await transact(async (tx, fx): Promise<{ kind: InviteJoinResult['kind']; id: string; communityId: string | null }> => {
    ...
    if (await getCommunityMember(tx, scope.community.id, me)) throw conflict('You are already a member of this community');
