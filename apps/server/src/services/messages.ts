    ...
    const rows = await dbx
      ...
      .where(
        and(
          ...
          inArray(chatMembers.userId, ids),
        ),
      );
...
export async function insertMessage(tx: Tx, input: CreateMessageInput): Promise<InsertedMessage> {
  ...
  const expiresAt =
    !serverMessage && chat!.disappearingSeconds
      ? new Date(createdAt.getTime() + chat!.disappearingSeconds * 1000)
      : null;
  ...
  const withheld =
    chat!.type === 'direct' && actorId ? [...(await blockersOf(tx, actorId, memberIds))] : [];
  const mentions = serverMessage
    ? []
    : await deriveMentions(
        tx,
        input.chatId,
        input.senderId,
        input.text,
        isChannel ? undefined : memberIds,
      );
  ...
  if (withheld.length) {
    await tx
      ...
      .values(withheld.map((userId) => ({ userId, messageId: message!.id })));
  }
  ...
  if (input.senderId) {
    const rows = await bumpMarks(tx, input.chatId, [input.senderId], {
      read: seq,
      delivered: seq,
  ...
  if (!isChannel)
    changedMarks.push(
      ...(await markDeliveredForOnlineRecipients(tx, { chatId: input.chatId, seq, recipientIds })),
    );
    ...
    publish(fx: Effects) {
      ...
      if (!isChannel)
        fx.watermarks({
          chatId: input.chatId,
          prevLastSeq,
          members: changedMarks,
          skipUserIds: withheld,
        });
      fx.domain('message.created', {
        message: row,
        chat: chatRow,
        recipientIds,
        withheldUserIds: withheld,
      });
    },
  };
}
...
export function buildPollDefinition(input: {
  question: string;
  options: string[];
...
export async function loadVisibleMessage(
  ...
  let q = dbx
    ...
    .innerJoin(
      chatMembers,
      and(eq(chatMembers.chatId, messages.chatId), eq(chatMembers.userId, viewerId)),
    )
...
export async function resolveReplyTarget(
  dbx: DbOrTx,
  senderId: string,
  chat: Pick<ChatRow, 'id' | 'type'>,
  replyToId: string,
): Promise<MessageRow> {
  const quotable = (m: MessageRow) => {
    if (m.type === 'system' || m.type === 'call' || m.deletedAt)
      throw badRequest('This message cannot be replied to');
    return m;
  };
  const same = await loadVisibleMessage(dbx, senderId, replyToId, { chatId: chat.id }).catch(
    () => null,
  );
  ...
  if (chat.type === 'direct') {
    ...
    if (
      peer &&
      other &&
      other.chat.type === 'group' &&
      other.message.senderId === peer.userId &&
      !other.member.leftAt
    ) {
...
export async function toMessages(
  ...
  const replyRows = replyIds.length
    ? await dbx.select().from(messages).where(inArray(messages.id, replyIds))
    : [];
  const replyById = new Map(replyRows.map((r) => [r.id, r]));

  const chatTypes = new Map(opts.chatTypes ?? []);
  const missingChats = uniq([
    ...rows.map((r) => r.chatId),
    ...replyRows.map((r) => r.chatId),
  ]).filter((id) => !chatTypes.has(id));
  if (missingChats.length) {
    for (const c of await dbx
      .select({ id: chats.id, type: chats.type })
      .from(chats)
      .where(inArray(chats.id, missingChats)))
      chatTypes.set(c.id, c.type);
  }
  const isChannel = (chatId: string) => chatTypes.get(chatId) === 'channel';
  const chatOf = new Map(rows.map((r) => [r.id, r.chatId]));

  const mediaRows = await loadMediaMap(dbx, [
    ...live.map((r) => r.mediaId),
    ...replyRows.map((r) => (r.deletedAt ? null : r.mediaId)),
  ]);
  const statusIds = live
    .map((r) => r.metadata?.statusReply?.statusId)
  ...
  const reactionGroups = new Map<
    string,
    { emoji: string; count: number; first: number; userIds: string[] }[]
  >();
  ...
  if (!opts.fresh && reactable.length) {
    ...
    for (const g of groups) {
      ...
      list.push({
        emoji: g.emoji,
        count: Number(g.n),
        first: new Date(g.first).getTime(),
        userIds: [],
      });
      ...
      const mine = await dbx
        ...
        .where(
          and(
            ...
            inArray(messageReactions.messageId, reactable),
          ),
        );
  ...
  if (!opts.fresh && pollIds.length) {
    ...
    if (viewerId) {
      ...
      for (const v of mine)
        myVotes.set(v.messageId, [...(myVotes.get(v.messageId) ?? []), v.optionId]);
    }
  }
  ...
  if (
    viewerId &&
    (reactable.some((id) => isDirect(chatOf.get(id)!) && reactionGroups.has(id)) ||
      pollIds.some((id) => isDirect(chatOf.get(id)!) && optionCounts.has(id)))
  ) {
    const rows = await dbx
  ...
  return rows.map((row) => {
    ...
    if (md.poll) {
      ...
      const shown = (optionId: string) =>
        (voters?.get(optionId) ?? []).filter((u) => !blockedByViewer.has(u));
      poll = {
        ...
        totalVoters: filterBlocked
          ? new Set(md.poll.options.flatMap((o) => shown(o.id))).size
          : (totalVoters.get(row.id) ?? 0),
      };
      if (viewerId) {
    ...
    const message: Message = {
      ...
      call: md.call ?? null,
      statusReply: md.statusReply
        ? toStatusReplyPayload(md.statusReply, statusMap.get(md.statusReply.statusId), now)
        : null,
      replyTo: replyTarget
        ? toPreview(replyTarget, chatTypes.get(replyTarget.chatId), mediaRows, now)
        : null,
...
export async function loadMessagePage(
  ...
  const serialized = await toMessages(
    dbx,
    viewerId,
    page,
    chatType ? { chatTypes: new Map([[chatId, chatType]]) } : {},
  );
...
export async function messageUpdatedPayloads(
  ...
  for (const e of except)
    exceptBy.set(e.message_id, [...(exceptBy.get(e.message_id) ?? []), e.user_id]);
  serialized.forEach((message, i) => {
    out.set(message.id, {
      chatId: rows[i]!.chatId,
      message,
      exceptUserIds: exceptBy.get(message.id) ?? [],
    });
  });
  return out;
}
...
export function publishMessagesRemoved(
  chatId: string,
  messageIds: string[],
  opts: { userId?: string } = {},
): void {
 ...
 */
export async function deleteForEveryoneTx(
  tx: Tx,
  fx: Effects,
  messageId: string,
): Promise<MessageRow> {
  ...
  if (current.type === 'system' || current.type === 'call')
  ...
  const pins = await tx
    .delete(chatPins)
    .where(eq(chatPins.messageId, messageId))
    .returning({ chatId: chatPins.chatId });
 ...
 */
export async function deleteForMeTx(
  tx: Tx,
  fx: Effects,
  userId: string,
  message: Pick<MessageRow, 'id' | 'chatId'>,
): Promise<void> {
  await tx.insert(messageHidden).values({ userId, messageId: message.id }).onConflictDoNothing();
  await tx
    .delete(starredMessages)
    .where(and(eq(starredMessages.userId, userId), eq(starredMessages.messageId, message.id)));
