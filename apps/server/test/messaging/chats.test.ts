...
describe('chats module (REST)', () => {
  ...
  describe('POST /chats/direct', () => {
    ...
    it('"Message yourself": one member row, peer = me, ticks = lastSeq, no calls', async () => {
      ...
      expect(chat).toMatchObject({
        peer: { id: me.id },
        memberCount: 1,
        permissions: { canSend: true, canCall: false },
      });
    ...
    it('unknown users → 404, invalid ids → 400, deleted users only with existing history', async () => {
      ...
      const res = await t
        ...
        .send({ type: 'text', text: 'hello?', clientId: 'x1' })
        .expect(403);
    ...
    it('can be opened with a blocked user (sending is gated: 403 blocked)', async () => {
      ...
      expect(chat.permissions).toMatchObject({
        canSend: false,
        canCall: false,
        canEditInfo: false,
      });
      const res = await t
        ...
        .send({ type: 'text', text: 'x', clientId: 'b1' })
        .expect(403);
  ...
  describe('GET /chats, GET /chats/:chatId', () => {
    it('lists archived and left chats, excludes deleted ones, newest activity first; 404 without a visible row', async () => {
      ...
      expect(list.find((c) => c.id === g1)).toMatchObject({
        membership: 'left',
        permissions: { canSend: false },
      });
      expect(list.find((c) => c.id === g2)).toMatchObject({
        isArchived: true,
        membership: 'active',
      });
  ...
  describe('ChatSummary.createdBy', () => {
    it('the creator of groups, channels and announcement groups for every viewer (also former members); null for direct chats', async () => {
      ...
      for (const u of [alice, bob, carol])
        expect((await summaryOf(t, u, g)).createdBy).toBe(alice.id);
      ...
      const created = await t
        .api(bob)
        .post('/api/groups')
        .send({ name: 'Via API', memberIds: [dave.id] })
        .expect(201);
  ...
  describe('PATCH /chats/:chatId/prefs', () => {
    it('pins at most MAX_PINNED_CHATS chats (409 limit_reached), emits chat:upsert to me', async () => {
      ...
      const over = await t
        .api(u)
        .patch(`/api/chats/${groups[3]}/prefs`)
        .send({ isPinned: true })
        .expect(409);
      ...
      expect(upserts.map((p) => p.chat.id)).toEqual([
        groups[0],
        groups[1],
        groups[2],
        groups[0],
    ...
    it('archives, mutes (until / forever / off) and marks unread without moving the read position', async () => {
      ...
      let s = (
        await t
          ...
          .send({ isArchived: true, mutedUntil: MUTE_FOREVER_ISO })
          .expect(200)
      ).body as ChatSummary;
      expect(s).toMatchObject({ isArchived: true, mutedUntil: MUTE_FOREVER_ISO });
      const until = new Date(Date.now() + 8 * 3600_000).toISOString();
      s = (
        await t.api(bob).patch(`/api/chats/${chatId}/prefs`).send({ mutedUntil: until }).expect(200)
      ).body;
      ...
      s = (
        await t
          .api(bob)
          .patch(`/api/chats/${chatId}/prefs`)
          .send({ mutedUntil: null, isArchived: false })
          .expect(200)
      ).body;
      ...
      expect(await summaryOf(t, alice, chatId)).toMatchObject({
        isArchived: false,
        mutedUntil: null,
        markedUnread: false,
      });
  ...
  describe('POST /chats/:chatId/clear', () => {
    it('clears history for me only, removes my stars in range, emits chat:cleared', async () => {
      ...
      const stars = await db
        ...
        .where(eq(starredMessages.messageId, m1.id));
  ...
  describe('DELETE /chats/:chatId', () => {
    ...
    it('direct chat: leaves the room, chat:removed; a new message unhides it without the old history', async () => {
      ...
      await t
        .api(u1)
        .patch(`/api/chats/${chatId}/prefs`)
        .send({ isPinned: true, markedUnread: true })
        .expect(200);
      const s1 = await t.connect(u1);
      const s2 = await t.connect(u2);
      ...
      expect(
        await db.select().from(starredMessages).where(eq(starredMessages.userId, u1.id)),
      ).toEqual([]);
  ...
  describe('PUT /chats/:chatId/disappearing', () => {
    it('direct chat: system message then chat:updated to the room; new messages expire; no-op when unchanged', async () => {
      ...
      await settle();
      expect(
        log.log.filter((e) => e.event === 'chat:updated' || e.event === 'message:new'),
      ).toEqual([]);
    ...
    it('groups follow canEditInfo; channels: admins, no system message; announcement groups and former members → 403', async () => {
      ...
      await settle();
      expect(log.of('chat:updated')).toEqual([
        { chatId: ch, changes: { disappearingSeconds: 86_400 } },
      ]);
      expect(log.of('message:new')).toEqual([]);
      expect(
        (await historyOf(t, alice, ch)).some((m) => m.system?.kind === 'disappearing_changed'),
      ).toBe(false);

      const { announcementChatId } = await createCommunity(alice);
      await t
        .api(alice)
        .put(`/api/chats/${announcementChatId}/disappearing`)
        .send({ seconds: 86_400 })
        .expect(403);
      bobSock.disconnect();
    });
  ...
  describe('GET /chats/:chatId/members', () => {
    ...
    it('channels and announcement groups: admins only; direct chats: both', async () => {
      ...
      expect(denied.body.error.code).toBe('forbidden');
      const list = (await t.api(carol).get(`/api/chats/${ch}/members`).expect(200))
        .body as ChatMember[];
      ...
      expect(
        (await t.api(alice).get(`/api/chats/${announcementChatId}/members`).expect(200)).body,
      ).toHaveLength(2);

      const d = await openDirect(t, alice, dave);
      const both = (await t.api(alice).get(`/api/chats/${d.id}/members`).expect(200))
        .body as ChatMember[];
  ...
  describe('GET /chats/:chatId/media', () => {
    it('media / docs / voice / links, newest first with a seq cursor, visible and not deleted', async () => {
      ...
      const img = await sendOk(t, alice, g, {
        type: 'image',
        mediaId: (await mkMedia(alice.id, 'image')).id,
        text: 'see https://example.com',
      });
      const vid = await sendOk(t, bob, g, {
        type: 'video',
        mediaId: (await mkMedia(bob.id, 'video')).id,
      });
      const file = await sendOk(t, alice, g, {
        type: 'file',
        mediaId: (await mkMedia(alice.id, 'file')).id,
      });
      const audio = await sendOk(t, alice, g, {
        type: 'audio',
        mediaId: (await mkMedia(alice.id, 'audio')).id,
      });
      const voice = await sendOk(t, bob, g, {
        type: 'voice',
        mediaId: (await mkMedia(bob.id, 'voice')).id,
      });
      const link = await sendOk(t, alice, g, 'read www.enbox.dev/docs today');
      await sendOk(t, alice, g, 'no links here');
      const gone = await sendOk(t, alice, g, {
        type: 'image',
        mediaId: (await mkMedia(alice.id, 'image')).id,
      });
      await t.api(alice).delete(`/api/messages/${gone.id}?for=everyone`).expect(204);
      const hidden = await sendOk(t, alice, g, {
        type: 'image',
        mediaId: (await mkMedia(alice.id, 'image')).id,
      });
      ...
      const get = async (q: string) =>
        ((await t.api(bob).get(`/api/chats/${g}/media${q}`).expect(200)).body as Message[]).map(
          (m) => m.id,
        );
      ...
      const full = (await t.api(bob).get(`/api/chats/${g}/media?kind=voice`).expect(200))
        .body as Message[];
      ...
      expect(
        (await t.api(carol).get(`/api/chats/${g}/media`).expect(200)).body as Message[],
      ).toEqual([]);
  ...
  describe('GET /chats/:chatId/media/counts', () => {
    it('counts every gallery kind with exactly the /media definitions and visibility', async () => {
      ...
      const hidden = await sendOk(t, alice, g, {
        type: 'file',
        mediaId: (await mkMedia(alice.id, 'file')).id,
        text: 'http://hidden.example',
      });
      await t.api(bob).delete(`/api/messages/${hidden.id}?for=me`).expect(204);

      const counts = async (u: TestUser) =>
        (await t.api(u).get(`/api/chats/${g}/media/counts`).expect(200)).body;
      const listed = async (u: TestUser) => {
        const out: Record<string, number> = {};
        for (const kind of ['media', 'docs', 'links', 'voice']) {
          out[kind] = (
            (await t.api(u).get(`/api/chats/${g}/media?kind=${kind}&limit=200`).expect(200))
              .body as Message[]
          ).length;
        }
        return out;
      ...
      expect(
        (await t.api(bob).get('/api/chats/nope/media/counts').expect(400)).body.error.code,
  ...
  describe('pins', () => {
    it('pin: system message then chat:pins; the oldest pin is replaced beyond MAX_PINNED_MESSAGES', async () => {
      ...
      const res = await t
        .api(alice)
        .post(`/api/chats/${g}/pins`)
        .send({ messageId: ms[0]!.id })
        .expect(200);
      ...
      expect(seq[0]!.payload.message.system).toEqual({
        kind: 'message_pinned',
        actorId: alice.id,
        messageId: ms[0]!.id,
      });
      ...
      expect(
        ((await t.api(bob).get(`/api/chats/${g}/pins`).expect(200)).body as Message[]).map(
          (m) => m.text,
        ),
      ).toEqual(['m2', 'm4']);
      bobSock.disconnect();
    ...
    it('validates the target: same chat, visible, not system/call/deleted', async () => {
      ...
      await t
        .api(alice)
        .post(`/api/chats/${g}/pins`)
        .send({ messageId: crypto.randomUUID() })
        .expect(404);
    ...
    it('permissions: canPin in groups; channels admins only without a system message; former members → 403', async () => {
      ...
      expect(
        (await t.api(carol).post(`/api/chats/${g}/pins`).send({ messageId: m.id }).expect(403)).body
          .error.code,
      ).toBe('not_member');
      ...
      expect((await historyOf(t, carol, d)).at(-1)!.system).toMatchObject({
        kind: 'message_pinned',
        actorId: dave.id,
      });
      bobSock.disconnect();
    ...
    it('former members get [] (no 403), even for pins inside their window; hidden rows and outsiders 404', async () => {
      ...
      expect(
        ((await t.api(carol).get(`/api/chats/${g}/pins`).expect(200)).body as Message[]).map(
          (m) => m.id,
        ),
      ).toEqual([inWindow.id]);
    ...
    it('pins are filtered by the viewer’s visibility', async () => {
      ...
      const rows = await db
        .select()
        .from(chatMembers)
        .where(and(eq(chatMembers.chatId, g), eq(chatMembers.userId, carol.id)));
      expect(rows).toHaveLength(1);
    });
  });
});
