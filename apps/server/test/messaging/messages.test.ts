...
const ageMessage = (id: string, ms: number) =>
  db
    .update(messages)
...
describe('messages module (REST)', () => {
  ...
  describe('PATCH /messages/:messageId (edit)', () => {
    it('sender within the window: text + mentions re-derived, edited_at, message:updated to the room', async () => {
      ...
      const res = await t
        ...
        .send({ text: `  hi ${mentionToken(carol.id)}  ` })
        .expect(200);
      const edited = res.body as Message;
      expect(edited).toMatchObject({
        id: m.id,
        text: `hi ${mentionToken(carol.id)}`,
        mentions: [carol.id],
        starred: false,
      });
      ...
      expect(upd[0]!.message).toMatchObject({
        id: m.id,
        text: `hi ${mentionToken(carol.id)}`,
        mentions: [carol.id],
      });
    ...
    it('rules: not the sender 403, window 410, text required, captions ≤ limit and may be emptied, non-editable types 400', async () => {
      ...
      const img = await sendOk(t, alice, g, {
        type: 'image',
        mediaId: (await mkMedia(alice.id)).id,
        text: 'caption',
      });
      await t
        .api(alice)
        .patch(`/api/messages/${img.id}`)
      ...
      await t
        .api(alice)
        .patch(`/api/messages/${crypto.randomUUID()}`)
        .send({ text: 'x' })
        .expect(404);
      ...
      const former = await t
        .api(bob)
        .patch(`/api/messages/${fm.id}`)
        .send({ text: 'x' })
        .expect(403);
  ...
  describe('DELETE /messages/:messageId', () => {
    it('for me: hidden for me only, my star removed, message:removed → my devices', async () => {
      ...
      expect(
        await db.select().from(starredMessages).where(eq(starredMessages.messageId, m.id)),
      ).toEqual([]);
    ...
    it('for everyone: scrubs content, reactions, pins, stars and votes; tombstone to the room; chat:pins; quotes show deleted', async () => {
      ...
      const m = await sendOk(t, alice, g, {
        type: 'image',
        mediaId: media.id,
        text: `pic for ${mentionToken(bob.id)}`,
      });
      ...
      await settle();
      const names = log.log
        .filter((e) => e.event === 'message:updated' || e.event === 'chat:pins')
        .map((e) => e.event);
      ...
      expect(
        await db.select().from(messageReactions).where(eq(messageReactions.messageId, m.id)),
      ).toEqual([]);
      expect(
        await db.select().from(starredMessages).where(eq(starredMessages.messageId, m.id)),
      ).toEqual([]);
      ...
      expect(history.find((x) => x.id === reply.id)!.replyTo).toMatchObject({
        id: m.id,
        deleted: true,
        text: null,
        media: null,
      });
      expect(history.find((x) => x.id === poll.id)).toMatchObject({
        poll: null,
  ...
  describe('reactions', () => {
    it('one reaction per user (replace / remove), myReaction in REST, message:updated to the room', async () => {
      ...
      let res = await t
        .api(bob)
        .put(`/api/messages/${m.id}/reaction`)
        .send({ emoji: '👍' })
        .expect(200);
      expect(res.body).toMatchObject({
        myReaction: '👍',
        reactions: [{ emoji: '👍', count: 1, userIds: [bob.id] }],
      });
    ...
    it('channels: reactions setting all / quick / none, anonymous userIds for everyone', async () => {
      ...
      const res = await t
        .api(bob)
        .put(`/api/messages/${post.id}/reaction`)
        .send({ emoji: '🦄' })
        .expect(200);
      expect(res.body).toMatchObject({
        myReaction: '🦄',
      ...
      await settle();
      expect(log.of('message:updated')[0]!.message.reactions).toEqual([
      ...
      const denied = await t
        .api(bob)
        .put(`/api/messages/${qp.id}/reaction`)
        .send({ emoji: '🦄' })
        .expect(403);
  ...
  describe('stars', () => {
    it('star / unstar privately (no events); starred list newest star first with chat previews', async () => {
      ...
      await settle();
      expect(la.log).toEqual([]);
      const list = (await t.api(bob).get('/api/messages/starred').expect(200))
        .body as MessageSearchResult[];
      expect(list.map((r) => r.message.id)).toEqual([m3.id, m2.id, m1.id]);
      expect(list[0]!.message.starred).toBe(true);
      expect(list[0]!.chat).toEqual({
        id: g,
        type: 'group',
        name: 'Stars group',
        avatarUrl: null,
      ...
      expect(list[1]!.chat).toMatchObject({
        id: d,
        type: 'direct',
        name: null,
        peer: { id: carol.id, displayName: 'Carol' },
      });
      ...
      expect(
        (
          (await t.api(bob).get('/api/messages/starred').expect(200)).body as MessageSearchResult[]
        ).map((r) => r.message.id),
      ).toEqual([m2.id, m1.id]);
    ...
    it('?chatId= narrows the starred list to one chat (same order, visibility and previews); 404 without a visible row', async () => {
      ...
      const starred = async (query = '') =>
        (
          (await t.api(u).get(`/api/messages/starred${query}`).expect(200))
            .body as MessageSearchResult[]
        ).map((r) => r.message.id);
      ...
      const one = (await t.api(u).get(`/api/messages/starred?chatId=${d}`).expect(200))
        .body as MessageSearchResult[];
      expect(one[0]).toMatchObject({
        message: { id: d1.id, starred: true },
        chat: { id: d, type: 'direct', peer: { id: carol.id } },
      });

      expect(
        (await t.api(u).get('/api/messages/starred?chatId=nope').expect(400)).body.error.code,
      ).toBe('validation_error');
      expect(
        (await t.api(u).get(`/api/messages/starred?chatId=${crypto.randomUUID()}`).expect(404)).body
          .error.code,
      ).toBe('not_found');
  ...
  describe('GET /messages/:messageId/info', () => {
    it('sender only; readBy / deliveredTo / pending from watermarks; 404 in channels', async () => {
      ...
      const info = (await t.api(alice).get(`/api/messages/${m.id}/info`).expect(200))
        .body as MessageInfo;
      ...
      const again = (await t.api(alice).get(`/api/messages/${m.id}/info`).expect(200))
        .body as MessageInfo;
      expect([
        ...again.readBy.map((r) => r.user.id),
        ...again.deliveredTo.map((r) => r.user.id),
        ...again.pending.map((u) => u.id),
    ...
    it('direct chats with read receipts off report delivery only', async () => {
      ...
      let info = (await t.api(u1).get(`/api/messages/${m.id}/info`).expect(200))
        .body as MessageInfo;
      ...
      expect((await t.api(u1).get(`/api/messages/${note.id}/info`).expect(200)).body).toEqual({
        messageId: note.id,
        readBy: [],
        deliveredTo: [],
        pending: [],
  ...
  describe('POST /messages/forward', () => {
    it('copies content to every target (forwardCount + 1, fresh poll ids, target timer, mentions re-derived) and fans out per copy', async () => {
      ...
      const statusReply = await sendOk(t, alice, d.id, {
        text: `re ${mentionToken(carol.id)}`,
        statusReplyToId: status.id,
      });
      const img = await sendOk(t, bob, src, {
        type: 'image',
        mediaId: (await mkMedia(bob.id)).id,
        text: 'photo',
      });
      const poll = await sendOk(t, bob, src, {
        type: 'poll',
        poll: { question: 'Q', options: ['x', 'y'] },
        replyToId: img.id,
      });
      await t
        .api(alice)
      ...
      const res = await t
        ...
        .send({
          clientId: 'fwd-1',
          messageIds: [img.id, poll.id, statusReply.id],
          chatIds: [t1, t2],
        })
        .expect(201);
      ...
      expect(out[1]!.poll!.options.map((o) => o.id)).not.toEqual(
        poll.poll!.options.map((o) => o.id),
      );
      expect(out[2]!.mentions).toEqual([carol.id]); // carol is in t1
      expect(out[5]!.mentions).toEqual([]); // not in the direct chat with dave
      await settle();
      expect(logC.of('message:new').map((p) => p.message.id)).toEqual(
        out.slice(0, 3).map((m) => m.id),
      );
      ...
      const again = await t
        .api(alice)
        .post('/api/messages/forward')
        .send({ clientId: 'fwd-2', messageIds: [out[0]!.id], chatIds: [src] })
        .expect(201);
    ...
    it('sources must be visible, not deleted, not system; targets need canSend (nothing is created on failure)', async () => {
      ...
      await t
        .api(carol)
        .post('/api/messages/forward')
        .send({ clientId: 'f1', messageIds: [hiddenFromCarol.id], chatIds: [carolChat] })
        .expect(404);
      const sys = (await historyOf(t, alice, g)).find((x) => x.type === 'system')!;
      await t
        .api(alice)
        .post('/api/messages/forward')
        .send({ clientId: 'f2', messageIds: [sys.id], chatIds: [g] })
        .expect(400);
      const del = await sendOk(t, alice, g, 'gone');
      await t.api(alice).delete(`/api/messages/${del.id}?for=everyone`).expect(204);
      await t
        .api(alice)
        .post('/api/messages/forward')
        .send({ clientId: 'f3', messageIds: [del.id], chatIds: [g] })
        .expect(400);

      const ok = await sendOk(t, alice, g, 'forward me');
      const adminsOnly = await createGroup(dave, [alice], {
      ...
      const denied = await t
        .api(alice)
        .post('/api/messages/forward')
        .send({ clientId: 'f4', messageIds: [ok.id], chatIds: [g, adminsOnly] })
        .expect(403);
      expect(denied.body.error.code).toBe('forbidden');
      ...
      await t
        .api(alice)
        .post('/api/messages/forward')
        .send({ clientId: 'f5', messageIds: [ok.id], chatIds: [crypto.randomUUID()] })
        .expect(404);
      await t
        .api(alice)
        .post('/api/messages/forward')
        .send({ clientId: 'f6', messageIds: [ok.id, ok.id], chatIds: [g] })
        .expect(400);
      await t
        .api(alice)
        .post('/api/messages/forward')
        .send({
          clientId: 'f7',
          messageIds: [ok.id],
          chatIds: Array.from({ length: 6 }, () => crypto.randomUUID()),
        })
        .expect(400);
    });
  });
  ...
  describe('PUT /messages/:messageId/vote', () => {
    it('single choice: replace / retract; unknown options and multiple ids → 400; message:updated', async () => {
      ...
      res = await t
        ...
        .send({ optionIds: [b] })
        .expect(200);
      expect(res.body.poll.options.map((o: { voteCount: number }) => o.voteCount)).toEqual([
        0, 1, 0,
      ]);
      await t
        .api(bob)
    ...
    it('multiple choice; channels are anonymous', async () => {
      const ch = await createChannel(alice);
      await follow(ch, [bob, carol]);
      const poll = await sendOk(t, alice, ch, {
        type: 'poll',
        poll: { question: 'Pick', options: ['1', '2', '3'], allowMultiple: true },
      });
      const ids = poll.poll!.options.map((o) => o.id);
      const res = await t
        .api(bob)
        .put(`/api/messages/${poll.id}/vote`)
        .send({ optionIds: [ids[0], ids[2]] })
        .expect(200);
      expect(res.body.poll).toMatchObject({ totalVoters: 1, myOptionIds: [ids[0], ids[2]] });
      await t
        .api(carol)
        .put(`/api/messages/${poll.id}/vote`)
        .send({ optionIds: [ids[2]] })
        .expect(200);
      ...
      expect(
        p.options.map((o: { voteCount: number; voterIds: string[] }) => [o.voteCount, o.voterIds]),
      ).toEqual([
        [1, []],
        [0, []],
        [2, []],
  ...
  describe('GET /search/messages', () => {
    it('finds only messages visible to me, optionally in one chat, newest first', async () => {
      ...
      const cap = await sendOk(t, bob, g, {
        type: 'image',
        mediaId: (await mkMedia(bob.id)).id,
        text: 'QUOKKA photo',
      });
      ...
      const res = (await t.api(bob).get('/api/search/messages?q=quokka').expect(200))
        .body as MessageSearchResult[];
      ...
      const inChat = (
        await t.api(bob).get(`/api/search/messages?q=quokka&chatId=${g}&limit=1`).expect(200)
      ).body as MessageSearchResult[];
    ...
    it('respects membership windows: nothing before joined_seq, nothing after left_seq, nothing cleared', async () => {
      ...
      const res = (await t.api(carol).get('/api/search/messages?q=zebra').expect(200))
        .body as MessageSearchResult[];
      ...
      expect(
        (
          (await t.api(bob).get('/api/search/messages?q=zebra').expect(200))
            .body as MessageSearchResult[]
        ).map((r) => r.message.id),
      ).toEqual([newer.id]);
    });
    ...
    it('search and the starred list use a fixed number of queries (no N+1)', async () => {
      ...
      const run = async (n: number) => {
        ...
        const search = await countQueries(() =>
          searchMessages(u.id, { q: `needle ${n}`, limit: 50 }),
        );
      ...
      const q = async (s: string) =>
        (
          (
            await t
              .api(dave)
              .get(`/api/search/messages?q=${encodeURIComponent(s)}`)
              .expect(200)
          ).body as MessageSearchResult[]
        ).map((r) => r.message.id);
