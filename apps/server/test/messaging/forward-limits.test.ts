...
describe('forward rate limit: per copy', () => {
  ...
  it('one forward cannot create more messages than the sendMessage window allows (50 × 5 → 400, nothing created)', async () => {
    ...
    const sources = (await historyOf(t, u, targets[0]!, '?limit=100')).filter((m) => m.type === 'text').map((m) => m.id);
    expect(sources).toHaveLength(50);
    const before = (await historyOf(t, u, targets[1]!, '?limit=100')).length;

    config.rateLimit = true;
    resetUserLimits();
    const res = await t.api(u).post('/api/messages/forward').send({ clientId: 'f5', messageIds: sources, chatIds: targets });
    ...
    await t.api(u).post(`/api/chats/${targets[0]}/messages`).send({ type: 'text', text: 'still fine', clientId: newClientId() }).expect(201);
  });
  ...
  it('two large forwards ~10 s apart cannot both pass: at most one window per window (≤ 120 copies in two windows)', async () => {
    ...
    let created = 0;
    try {
      const r1 = await t.api(me).post('/api/messages/forward').send({ clientId: newClientId(), messageIds: ids, chatIds: targets });
      if (r1.status < 300) created += (r1.body as unknown[]).length;
      Date.now = () => realNow() + 10_500; // the next 10 s window
      const r2 = await t.api(me).post('/api/messages/forward').send({ clientId: newClientId(), messageIds: ids, chatIds: targets });
      ...
      const r3 = await t.api(me).post('/api/messages/forward').send({ clientId: newClientId(), messageIds: ids.slice(0, 30), chatIds: targets });
      expect(r3.status).toBe(201);
      created += (r3.body as unknown[]).length;
      const r4 = await t.api(me).post('/api/messages/forward').send({ clientId: newClientId(), messageIds: ids.slice(0, 1), chatIds: targets });
