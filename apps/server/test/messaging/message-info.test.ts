...
describe('message info: only for those who may see the member list', () => {
  ...
  const exposed = (body: MessageInfo) => [
    ...body.readBy.map((r) => r.user.id),
    ...body.deliveredTo.map((r) => r.user.id),
    ...body.pending.map((u) => u.id),
  ];
  ...
  it('announcement group: a demoted admin cannot enumerate community members through the info of old posts', async () => {
    ...
    const community = (
      await t.api(owner).post('/api/communities').send({ name: 'Neighbourhood' }).expect(201)
    ).body as Community;
    await t
      .api(owner)
      .post(`/api/communities/${community.id}/members`)
      .send({ userIds: [bob.id, carol.id, dave.id] })
      .expect(200);
    await t
      .api(owner)
      .put(`/api/communities/${community.id}/members/${bob.id}/role`)
      .send({ role: 'admin' })
      .expect(204);
    ...
    expect(
      exposed((await t.api(bob).get(`/api/messages/${m.id}/info`).expect(200)).body as MessageInfo),
    ).toHaveLength(3);
    ...
    expect({
      ...
      exposed: res.status === 200 ? exposed(res.body as MessageInfo) : [],
    }).toEqual({
      status: 403,
      code: 'forbidden',
      exposed: [],
  ...
  it('active members of regular groups and direct chats keep seeing the info of their messages', async () => {
    ...
    const res = (await t.api(member).get(`/api/messages/${m.id}/info`).expect(200))
      .body as MessageInfo;
  ...
  it('keeps the 404 for channel posts', async () => {
    ...
    const channel = (await t.api(owner).post('/api/channels').send({ name: 'News' }).expect(201))
      .body as { id?: string; chat?: { id: string } };
