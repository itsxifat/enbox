...
describe('status updates', () => {
  ...
  const post = (u: TestUser, body: unknown, status = 201) =>
    t
      .api(u)
      .post('/api/status')
      .send(body as object)
  ...
  const feed = async (u: TestUser) =>
    (await t.api(u).get('/api/status/feed').expect(200)).body as StatusFeed;
  const audienceOf = async (id: string) => {
    const [row] = await db
      .select({ audience: statuses.audience })
      .from(statuses)
  ...
  const upload = async (u: TestUser, kind: 'image' | 'video') => {
    const res = await t
      ...
      .post('/api/media')
      .field('kind', kind)
      .attach('file', kind === 'image' ? PNG : WEBM, {
        filename: kind === 'image' ? 'p.png' : 'v.webm',
  ...
  describe('audience', () => {
    ...
    it('contacts_except / only_share_with (lists ∩ contacts)', async () => {
      ...
      await setSettings(alice!, {
        statusPrivacy: 'contacts_except',
        statusExcludeUserIds: [carol!.id, stranger!.id],
      });
      expect(await audienceOf((await post(alice!, { type: 'text', text: 'a' })).body.id)).toEqual(
        sorted(bob!, dave!),
      );

      await setSettings(alice!, {
        statusPrivacy: 'only_share_with',
        statusOnlyShareWithUserIds: [carol!.id, stranger!.id],
      });
      expect(await audienceOf((await post(alice!, { type: 'text', text: 'b' })).body.id)).toEqual(
        sorted(carol!),
      );
      ...
      expect(await audienceOf((await post(alice!, { type: 'text', text: 'd' })).body.id)).toEqual(
        sorted(bob!, carol!, dave!),
      );
  ...
  describe('POST /status', () => {
    it('media statuses: my upload of the matching kind, caption trimmed, no colors', async () => {
      ...
      const s1 = (await post(alice!, { type: 'image', mediaId: img.id, text: ' look ' }))
        .body as Status;
      expect(s1).toMatchObject({
        ...
        font: null,
        media: { id: img.id, kind: 'image', url: img.url },
      });
      ...
      expect((await post(alice!, { type: 'video', mediaId: img.id }, 400)).body.error.code).toBe(
        'validation_error',
      );
      expect((await post(bob!, { type: 'image', mediaId: img.id }, 404)).body.error.code).toBe(
        'not_found',
      );
      expect(
        (await post(alice!, { type: 'image', mediaId: crypto.randomUUID() }, 404)).body.error.code,
      ).toBe('not_found');
    });
    ...
    it('validates bodies (discriminated, strict)', async () => {
      ...
      for (const body of bad)
        expect((await post(alice!, body, 400)).body.error.code).toBe('validation_error');
      const ok = (
        await post(alice!, { type: 'text', text: 'styled', backgroundColor: '#112233', font: 4 })
      ).body as Status;
  ...
  describe('feed, views, reactions, viewers', () => {
    ...
    beforeAll(async () => {
      [alice, bob, carol, dave, stranger] = (await people(5)) as [
        TestUser,
        TestUser,
        TestUser,
        TestUser,
    ...
    it('groups other users by author: unviewed first, then most recent; mine oldest first', async () => {
      ...
      f = await feed(alice);
      expect(f.updates.find((u) => u.user.id === bob.id)!.statuses.map((s) => s.id)).toEqual([
        b1.id,
      ]);
    ...
    it('views: audience only; the first view notifies the author; read receipts off records silently', async () => {
      ...
      expect(
        await db.select().from(statusViews).where(eq(statusViews.statusId, s.id)),
      ).toHaveLength(1);
      ...
      expect((await t.api(bob).post('/api/status/nope/view').expect(400)).body.error.code).toBe(
        'validation_error',
      );
      ...
      expect(
        (await feed(dave)).updates
          .find((u) => u.user.id === alice.id)!
          .statuses.find((x) => x.id === s.id)!.viewed,
      ).toBe(true);
      expect((await feed(alice)).mine.find((x) => x.id === s.id)!.viewCount).toBe(1);
      const viewers = (await t.api(alice).get(`/api/status/${s.id}/viewers`).expect(200))
        .body as StatusViewer[];
      ...
      const again = (await t.api(alice).get(`/api/status/${s.id}/viewers`).expect(200))
        .body as StatusViewer[];
    ...
    it('reactions: one emoji, audience only, replaced; the author gets status:viewed with it', async () => {
      ...
      expect(ra.of('status:viewed')[1]!.viewer).toMatchObject({
        user: { id: bob.id },
        reaction: '❤️',
        viewedAt,
      });
      ...
      for (const emoji of ['hi', '👍👍', '']) {
        expect(
          (await t.api(bob).put(`/api/status/${s.id}/reaction`).send({ emoji }).expect(400)).body
            .error.code,
        ).toBe('validation_error');
      }
      expect(
        (await t.api(alice).put(`/api/status/${s.id}/reaction`).send({ emoji: '👍' }).expect(403))
          .body.error.code,
      ).toBe('forbidden');
      ...
      expect(
        (
          (await t.api(alice).get(`/api/status/${s.id}/viewers`).expect(200)).body as StatusViewer[]
        ).map((v) => v.user.id),
      ).not.toContain(dave.id);
      await setSettings(dave, { readReceipts: true });
    });
    ...
    it('status:viewed carries firstView (new view vs reaction update) and the author’s current viewCount', async () => {
      ...
      expect(events()[0]).toMatchObject({
        viewer: { user: { id: bob.id }, reaction: null },
        firstView: true,
        viewCount: 1,
      });
      ...
      expect(events()[1]).toMatchObject({
        viewer: { user: { id: bob.id }, reaction: '❤️' },
        firstView: false,
        viewCount: 1,
      });
      ...
      try {
        ...
        expect(events()[2]).toMatchObject({
          viewer: { user: { id: carol.id }, reaction: '👍' },
          firstView: true,
          viewCount: 2,
        });
        await t.api(carol).put(`/api/status/${s.id}/reaction`).send({ emoji: '😂' }).expect(204);
    ...
    it('viewers: author only (audience 403, others 404)', async () => {
      ...
      expect(
        (await t.api(bob).get(`/api/status/${s.id}/viewers`).expect(403)).body.error.code,
      ).toBe('forbidden');
  ...
  describe('deletion and expiry', () => {
    it('DELETE: author only; status:deleted → audience + me; replies become unavailable; media freed', async () => {
      ...
      expect((await loadMessages(db, bob!.id, [message.id]))[0]!.statusReply).toMatchObject({
        available: true,
        mediaUrl: img.url,
      });

      expect((await t.api(bob!).delete(`/api/status/${s.id}`).expect(403)).body.error.code).toBe(
        'forbidden',
      );
    ...
    it('the expiry job purges expired statuses (views cascade) in batches', async () => {
      ...
      const ids: string[] = [];
      for (let i = 0; i < 5; i++)
        ids.push(((await post(alice!, { type: 'text', text: `e${i}` })).body as Status).id);
      ...
      expect((await feed(bob!)).updates[0]!.statuses.map((s) => s.id)).toEqual([
        ...ids.slice(3),
        keep,
      ]);
      ...
      expect(
        (
          await db
            ...
            .from(statuses)
            .where(inArray(statuses.id, [...ids, keep]))
        )
          .map((r) => r.id)
          .sort(),
      ).toEqual([...ids.slice(3), keep].sort());
