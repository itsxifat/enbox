...
describe('calls: group', () => {
  ...
  it('rings a subset, others join, invites, participant events go to call sockets only, ends when fewer than 2 remain', async () => {
    ...
    const call = await ackCall(a!, 'call:start', {
      chatId,
      type: 'audio',
    ...
    await until(
      () => rb.of('call:incoming').length === 1 && rc.of('call:incoming').length === 1,
      3000,
      'incoming',
    );
    expect(rb.of('call:incoming')[0]!.chat).toEqual({
      id: chatId,
      type: 'group',
      name: 'Team',
      avatarUrl: null,
    ...
    await until(
      () => rd.of('message:new').some((m) => m.message.type === 'call'),
      3000,
      'call message',
    );
    expect(rd.of('message:new').find((m) => m.message.type === 'call')!.message.call).toMatchObject(
      { isGroup: true, status: 'ringing' },
    );
    await settle();
    ...
    expect(
      (await t.api(dave).get('/api/calls/active').expect(200)).body.map(
        (x: { id: string }) => x.id,
      ),
    ).toContain(call.id);
    ...
    expect(joined.participants.find((p) => p.userId === dave!.id)).toMatchObject({
      status: 'joined',
      audioMuted: true,
      videoOff: true,
    });
    ...
    await until(
      () => rb.of('call:updated').some((p) => p.call.status === 'ongoing'),
      3000,
      'bob call:updated',
    );
    await until(
      () => re.of('message:updated').some((p) => p.message.call?.status === 'ongoing'),
      3000,
      'message ongoing',
    );
    ...
    await until(
      () =>
        ra.of('call:participant-joined').length === 1 &&
        rd.of('call:participant-joined').length === 1,
      3000,
      'bob joined',
    );
    ...
    clearAll(recs);
    const invited = await ackCall(b!, 'call:invite', {
      callId: call.id,
      userIds: [erin!.id, dave!.id, alice!.id],
    });
    ...
    await until(
      () => ra.of('call:updated').some((p) => statusOf(p.call, erin!.id) === 'invited'),
      3000,
      'call:updated erin',
    );
    ...
    await until(
      async () =>
        (await partOf(call.id, erin!.id)).status === 'declined' &&
        (await partOf(call.id, carol!.id)).status === 'declined',
      3000,
      'declines',
    );
    clearAll(recs);
    ...
    await until(
      () =>
        ra.of('call:participant-left').length === 1 && rb.of('call:participant-left').length === 1,
      3000,
      'dave left',
    );
    expect(ra.of('call:participant-left')[0]).toEqual({ callId: call.id, userId: dave!.id });
    await until(
      () => ra.of('call:updated').some((p) => statusOf(p.call, dave!.id) === 'left'),
      3000,
      'dave left update',
    );
    ...
    await until(
      () => re.of('message:updated').some((p) => p.message.call?.status === 'ended'),
      3000,
      'message ended',
    );
    expect(ended.find((x) => x.callId === call.id)!.participantIds.sort()).toEqual(
      [alice!.id, bob!.id, carol!.id, dave!.id, erin!.id].sort(),
    );
  });
  ...
  it('ringing group call: initiator leaving cancels; all declined → declined; decline + timeout → missed', async () => {
    ...
    expect(cancelled.participants.map((p) => p.userId).sort()).toEqual(
    ...
    await until(
      () => ra!.of('call:updated').some((p) => statusOf(p.call, bob!.id) === 'declined'),
      3000,
      'bob declined',
    );
  ...
  it('join / accept / invite validations', async () => {
    ...
    expect((await ackError(c!, 'call:invite', { callId: call.id, userIds: [dave!.id] })).code).toBe(
      'forbidden',
    );
    expect((await ackError(s!, 'call:invite', { callId: call.id, userIds: [dave!.id] })).code).toBe(
      'not_found',
    );
    expect(
      (await ackError(a!, 'call:invite', { callId: crypto.randomUUID(), userIds: [dave!.id] }))
        .code,
    ).toBe('not_found');
    expect((await ackError(a!, 'call:invite', { callId: call.id, userIds: [] })).code).toBe(
      'validation_error',
    );
    ...
    await until(
      async () => (await partOf(call.id, bob!.id)).status === 'declined',
      3000,
      'declined',
    );
    ...
    send(c!, 'call:decline', { callId: call.id });
    await until(
      async () => (await partOf(call.id, carol!.id)).status === 'declined',
      3000,
      'carol declined',
    );
    ...
    rc!.clear();
    rb!.clear();
    const reinvited = await ackCall(b!, 'call:invite', {
      callId: call.id,
      userIds: [carol!.id, dave!.id, stranger!.id, erin!.id],
    });
    ...
    expect((await partOf(call.id, carol!.id)).invitedAt.getTime()).toBeGreaterThan(
      before.getTime(),
    );
    ...
    await transact(async (tx, fx) => {
      await upsertMembership(tx, fx, {
        ...
        userId: carol!.id,
        reason: 'left',
        systemEvent: { kind: 'member_left', actorId: carol!.id },
      });
    });
    await until(
      async () => (await partOf(call.id, carol!.id)).status === 'missed',
      3000,
      'carol forced out',
    );
    ...
    expect((await ackError(a!, 'call:invite', { callId: call.id, userIds: [bob!.id] })).code).toBe(
      'expired',
    );
    send(e!, 'call:leave', { callId: busyCall.id });
    send(f!, 'call:leave', { callId: busyCall.id });
    await until(
      async () => (await callRow(busyCall.id)).status === 'ended',
      3000,
      'busy call ended',
    );
    void d;
  ...
  it(`at most ${MAX_CALL_PARTICIPANTS} joined participants; ringing everyone needs userIds in big groups`, async () => {
    ...
    expect(statusOf(await ackCall(last, 'call:join', { callId: call.id }), others.at(-1)!.id)).toBe(
      'joined',
    );
  });
  ...
  it('forced leave when a participant leaves or is removed from the group (member.left)', async () => {
    ...
    await transact(async (tx, fx) => {
      await upsertMembership(tx, fx, {
        ...
        userId: dave!.id,
        reason: 'left',
        systemEvent: { kind: 'member_left', actorId: dave!.id },
      });
    });
  ...
  it('permissions: onlyAdminsCanSend groups, channels, former members', async () => {
    const [alice, bob, carol] = await users(3);
    const strict = await createGroup(alice!, [bob!, carol!], {
    ...
    expect((await ackError(b!, 'call:start', { chatId: strict, type: 'audio' })).code).toBe(
      'forbidden',
    );
    expect((await ackError(a!, 'call:start', { chatId: channel, type: 'audio' })).code).toBe(
      'forbidden',
    );
    const call = await ackCall(a!, 'call:start', {
      chatId: strict,
    ...
    expect(
      (await ackError(a!, 'call:start', { chatId: strict, type: 'audio', userIds: [bob!.id] }))
        .code,
