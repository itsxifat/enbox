...
export async function device(
  t: TestServer,
  user: TestUser,
): Promise<{ user: TestUser; socket: TestSocket }> {
...
export function rawAck<T = unknown>(
  socket: TestSocket,
  ...
  return new Promise((resolve, reject) => {
    (socket as unknown as ClientSocket)
      .timeout(5000)
      .emit(event as string, payload, (err: unknown, res: Ack<T>) =>
...
export const statusOf = (call: Call, userId: string) =>
  call.participants.find((p) => p.userId === userId)?.status;
