...
import { expectNoEvent, sleep, startTestServer, type TestServer, type TestSocket, type TestUser } from '../helpers.js';
import { block, createDirect, recordEvents, saveContact, setSettings, settle } from '../services/fixtures.js';
...
describe('calls: 1:1 signaling', () => {
  ...
  async function connectRecording(user: TestUser) {
    const socket = ioClient(t.url, { auth: { token: user.token }, transports: ['websocket'], forceNew: true, reconnection: false }) as TestSocket;
  ...
  it('rings every callee device, answers on one, relays signals only between call sockets and ends with a duration', async () => {
    ...
    expect(call).toMatchObject({ chatId, type: 'video', isGroup: false, initiatorId: alice.id, status: 'ringing', answeredAt: null, endedAt: null, durationSec: null });
    expect(call.participants).toEqual([
      expect.objectContaining({ userId: alice.id, status: 'joined', audioMuted: false, videoOff: false, screenSharing: false }),
    ...
    for (const r of [rb1, rb2]) {
      ...
      expect(inc.chat).toMatchObject({ id: chatId, type: 'direct', name: null, avatarUrl: null, memberCount: 2 });
      expect(inc.chat.peer?.id).toBe(alice.id);
      expect(r.of('message:new')[0]!.message).toMatchObject({
        type: 'call',
        senderId: alice.id,
        call: { callId: call.id, callType: 'video', isGroup: false, initiatorId: alice.id, status: 'ringing', durationSec: null },
      });
    }
    ...
    expect(ringing.payload).toMatchObject({ chatId, callerId: alice.id, callType: 'video', isGroup: false, userIds: [bob.id], silentUserIds: [] });

    // A device of Bob rings → Alice sees "Ringing".
    send(b1, 'call:ringing', { callId: call.id });
    await until(() => ra1.of('call:updated').some((p) => statusOf(p.call, bob.id) === 'ringing'), 3000, 'ringing');
    ...
    expect(accepted.participants.find((p) => p.userId === bob.id)).toMatchObject({ status: 'joined', audioMuted: true, videoOff: false });
    ...
    expect(ra1.names().filter((n) => n.startsWith('call:') || n === 'message:updated')).toEqual(['call:participant-joined', 'call:updated', 'message:updated']);
    ...
    expect(ra1.of('message:updated')[0]!.message.call).toMatchObject({ status: 'ongoing', durationSec: null });
    await settle();
    ...
    expect(stopped.payload).toMatchObject({ userId: bob.id, reason: 'answered_elsewhere', finalStatus: 'joined' });
    ...
    for (const r of [ra1, ra2, rb1, rb2]) r.clear();
    send(b2, 'call:signal', { callId: call.id, toUserId: alice.id, signal: { type: 'offer', sdp: 'v=0 offer' } });
    await until(() => ra1.of('call:signal').length === 1, 3000, 'offer');
    expect(ra1.of('call:signal')[0]).toEqual({ callId: call.id, fromUserId: bob.id, signal: { type: 'offer', sdp: 'v=0 offer' } });
    send(a1, 'call:signal', { callId: call.id, toUserId: bob.id, signal: { type: 'answer', sdp: 'v=0 answer' } });
    send(a1, 'call:signal', {
      callId: call.id,
      toUserId: bob.id,
      signal: { type: 'candidate', candidate: { candidate: 'candidate:1 1 udp 1 1.2.3.4 5 typ host', sdpMid: '0', sdpMLineIndex: 0 } },
    });
    ...
    send(b1, 'call:signal', { callId: call.id, toUserId: alice.id, signal: { type: 'offer', sdp: 'intruder' } });
    send(a2, 'call:signal', { callId: call.id, toUserId: bob.id, signal: { type: 'offer', sdp: 'intruder' } });
    send(a1, 'call:signal', { callId: call.id, toUserId: alice.id, signal: { type: 'offer', sdp: 'self' } });
    await settle(250);
    ...
    send(b2, 'call:media', { callId: call.id, audioMuted: false, videoOff: true, screenSharing: true });
    await until(() => ra1.of('call:media').length === 1, 3000, 'call:media');
    expect(ra1.of('call:media')[0]).toEqual({ callId: call.id, audioMuted: false, videoOff: true, screenSharing: true, userId: bob.id });
    await until(() => rb2.of('call:media').length === 1, 3000, 'call:media echo');
    expect(await partOf(call.id, bob.id)).toMatchObject({ audioMuted: false, videoOff: true, screenSharing: true });
    send(b1, 'call:media', { callId: call.id, audioMuted: true, videoOff: true, screenSharing: false });
    await settle(200);
    ...
    await until(() => [ra2, rb1, rb2].every((r) => r.of('call:ended').length === 1), 3000, 'call:ended');
    ...
    expect(rb2.names().indexOf('call:participant-left')).toBeLessThan(rb2.names().indexOf('call:ended'));
    ...
    expect(domainOf(call.id).find((d) => d.event === 'call.ended')!.payload).toMatchObject({ status: 'ended', participantIds: expect.arrayContaining([alice.id, bob.id]) });
    const row = await callRow(call.id);
    expect(row.status).toBe('ended');
    expect(row.endedAt).not.toBeNull();

    // The call rooms were left: nothing is relayed any more.
    send(b2, 'call:signal', { callId: call.id, toUserId: alice.id, signal: { type: 'offer', sdp: 'late' } });
  ...
  it('declining on one device stops every device and ends a 1:1 call as declined', async () => {
    ...
    expect(call.participants[0]).toMatchObject({ userId: alice.id, audioMuted: false, videoOff: true }); // audio: video off by default
    await until(() => rb1.of('call:incoming').length === 1 && rb2.of('call:incoming').length === 1, 3000, 'incoming');
    ...
    await until(() => ra1.of('message:updated').length === 1, 3000, 'message:updated');
    expect(ra1.of('message:updated')[0]!.message.call).toMatchObject({ status: 'declined', durationSec: null });
  ...
  it('the caller hanging up while ringing cancels the call and stops the ring', async () => {
    ...
    expect(domainOf(call.id).find((d) => d.event === 'call.ring-stopped')!.payload).toMatchObject({ userId: bob.id, reason: 'cancelled', finalStatus: 'missed' });
    const msg = await db.select().from(messages).where(eq(messages.id, (await callRow(call.id)).messageId!));
  ...
  it('ring timeout: the callee becomes missed and the call ends missed (with ring-stop + domain events)', async () => {
    ...
    expect(domainOf(call.id)[1]!.payload).toMatchObject({ reason: 'timeout', finalStatus: 'missed' });
  ...
  it('cross-calls: a second call:start in the chat → conflict with details.callId, then accept it', async () => {
    ...
    expect(await ackError(a1, 'call:start', { chatId, type: 'audio' })).toMatchObject({ code: 'conflict', details: { callId: call.id } });
  ...
  it('call:rejoin within the grace (other session) re-binds the call socket; peers get participant-joined', async () => {
    ...
    send(a1, 'call:signal', { callId: call.id, toUserId: bob.id, signal: { type: 'offer', sdp: 'renegotiate' } });
  ...
  it('late devices get call:incoming after ready while still ringing (not once answered, never when hidden)', async () => {
    ...
    expect(late.rec.of('call:incoming')[0]).toMatchObject({ call: { id: call.id, status: 'ringing' }, silent: false, caller: { id: alice.id } });
  ...
  it('blocked callee: hidden participant, never rung or notified; the call ends missed at the timeout', async () => {
    ...
    const row = await callRow(call.id);
    const [hidden] = await db.select().from(messageHidden).where(and(eq(messageHidden.userId, bob.id), eq(messageHidden.messageId, row.messageId!)));
    expect(hidden).toBeDefined();
    const [member] = await db.select().from(chatMembers).where(and(eq(chatMembers.chatId, chatId), eq(chatMembers.userId, bob.id)));
    ...
    expect(aliceLog[0]).toMatchObject({ call: { id: call.id }, direction: 'outgoing', outcome: 'unanswered' });
  ...
  it('eligibility: blocked by me, deleted peer, self chat, non-member, invalid payloads', async () => {
    ...
    expect(await ackError(a1, 'call:start', { chatId, type: 'audio' })).toMatchObject({ code: 'blocked' });

    const carol = await t.createUser();
    const ac = await createDirect(alice, carol);
    await db.update(users).set({ deletedAt: new Date() }).where(eq(users.id, carol.id));
    expect(await ackError(a1, 'call:start', { chatId: ac, type: 'audio' })).toMatchObject({ code: 'forbidden' });

    const self = await createDirect(alice, alice);
    expect(await ackError(a1, 'call:start', { chatId: self, type: 'audio' })).toMatchObject({ code: 'forbidden' });

    const dave = await t.createUser();
    const erin = await t.createUser();
    const de = await createDirect(dave, erin);
    expect(await ackError(a1, 'call:start', { chatId: de, type: 'audio' })).toMatchObject({ code: 'not_found' });
    expect(await ackError(a1, 'call:start', { chatId: crypto.randomUUID(), type: 'audio' })).toMatchObject({ code: 'not_found' });

    expect(await ackError(a1, 'call:start', { chatId: 'nope', type: 'audio' })).toMatchObject({ code: 'validation_error' });
    expect(await ackError(a1, 'call:start', { chatId: de, type: 'hologram' })).toMatchObject({ code: 'validation_error' });
    expect(await ackError(a1, 'call:accept', { callId: 'x' })).toMatchObject({ code: 'validation_error' });
    expect(await ackError(a1, 'call:accept', { callId: crypto.randomUUID() })).toMatchObject({ code: 'not_found' });

    // Direct chats ring only the peer.
    const frank = await t.createUser();
    const af = await createDirect(alice, frank);
    expect(await ackError(a1, 'call:start', { chatId: af, type: 'audio', userIds: [dave.id] })).toMatchObject({ code: 'validation_error' });
    ...
    expect((await ackError(d1, 'call:invite', { callId: ok.id, userIds: [erin.id] })).code).toBe('not_found');
    // Invites are for group calls.
    expect((await ackError(a1, 'call:invite', { callId: ok.id, userIds: [frank.id] })).code).toBe('forbidden');
  ...
  it('silenceUnknownCallers: silent ring for non-contacts (no call:ringing), normal for contacts', async () => {
    ...
    expect(domainOf(call.id)[0]!.payload).toMatchObject({ userIds: [bob.id], silentUserIds: [bob.id] });
    ra1.clear();
    send(b1, 'call:ringing', { callId: call.id });
    await settle(250);
    ...
    await until(async () => (await partOf(second.id, bob.id)).status === 'ringing', 3000, 'ringing');
  ...
  it('events on the incoming side reach every device in matrix order (chat:upsert → message:new → call:incoming)', async () => {
    ...
    const seen = late.rec.names().filter((n): n is keyof ServerToClientEvents => ['chat:upsert', 'message:new', 'call:incoming'].includes(n));
