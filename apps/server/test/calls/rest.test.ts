...
describe('calls: REST', () => {
  ...
  const waitStatus = (callId: string, status: Call['status']) =>
    until(async () => (await callRow(callId)).status === status, 3000, `call ${status}`);
  ...
  describe('GET /calls (call log)', () => {
    ...
    it('lists my calls newest first with per-viewer direction/outcome and the chat', async () => {
      ...
      expect(log.map((e) => e.call.id)).toEqual([
        ids.live,
        ids.busy,
        ids.group,
        ids.unanswered,
      ...
      expect(byId[ids.answered!]).toMatchObject({
        direction: 'outgoing',
        outcome: 'answered',
        call: { status: 'ended', type: 'video' },
      });
      ...
      expect(byId[ids.group!]).toMatchObject({
        direction: 'incoming',
        outcome: 'answered',
        call: { isGroup: true },
      });
      ...
      expect(byId[ids.answered!]!.chat).toMatchObject({
        id: ab,
        type: 'direct',
        name: null,
        avatarUrl: null,
      ...
      expect(byId[ids.group!]!.chat).toEqual({
        id: group,
        type: 'group',
        name: 'Trio',
        avatarUrl: null,
      ...
      const bobLog = Object.fromEntries(
        ((await t.api(bob).get('/api/calls').expect(200)).body as CallLogEntry[]).map((e) => [
          e.call.id,
          e,
        ]),
      );
      ...
      expect(bobLog[ids.unanswered!]).toMatchObject({
        direction: 'outgoing',
        outcome: 'unanswered',
      });
      ...
      expect(carolLog.map((e) => [e.call.id, e.direction, e.outcome])).toEqual([
        [ids.group, 'outgoing', 'answered'],
      ]);
      const daveLog = (await t.api(dave).get('/api/calls').expect(200)).body as CallLogEntry[];
      expect(daveLog.map((e) => [e.call.id, e.direction, e.outcome])).toEqual([
        [ids.busy, 'outgoing', 'unanswered'],
      ]);
    });

    it('pages with limit and the before cursor', async () => {
      const first = (await t.api(alice).get('/api/calls?limit=3').expect(200))
        .body as CallLogEntry[];
      expect(first.map((e) => e.call.id)).toEqual([ids.live, ids.busy, ids.group]);
      const next = (
        await t
          .api(alice)
          .get(`/api/calls?limit=3&before=${encodeURIComponent(first[2]!.call.createdAt)}`)
          .expect(200)
      ).body as CallLogEntry[];
      ...
      const last = (
        await t
          .api(alice)
          .get(`/api/calls?before=${encodeURIComponent(next[2]!.call.createdAt)}`)
          .expect(200)
      ).body as CallLogEntry[];
      ...
      expect((await t.api(alice).get('/api/calls?limit=0').expect(400)).body.error.code).toBe(
        'validation_error',
      );
      expect(
        (await t.api(alice).get('/api/calls?before=yesterday').expect(400)).body.error.code,
    ...
    it('DELETE /calls/:callId hides one ended call from my log only; live → 409; unknown/foreign → 404', async () => {
      ...
      expect(
        ((await t.api(bob).get('/api/calls').expect(200)).body as CallLogEntry[]).map(
          (e) => e.call.id,
        ),
      ).toContain(ids.declined);
      expect(
        (await t.api(alice).delete(`/api/calls/${ids.declined}`).expect(404)).body.error.code,
      ).toBe('not_found');
      expect(
        (await t.api(alice).delete(`/api/calls/${ids.live}`).expect(409)).body.error.code,
      ).toBe('conflict');
      ...
      expect((await t.api(alice).delete('/api/calls/not-a-uuid').expect(400)).body.error.code).toBe(
        'validation_error',
      );
    });
    ...
    it('GET /calls/:callId returns my log entry (same serialization); hidden / not mine / unknown → 404', async () => {
      ...
      for (const entry of log) {
        expect((await t.api(alice).get(`/api/calls/${entry.call.id}`).expect(200)).body).toEqual(
          entry,
        );
      }
      ...
      expect((await t.api(carol).get(`/api/calls/${ids.group}`).expect(200)).body).toMatchObject({
        direction: 'outgoing',
        outcome: 'answered',
        chat: { name: 'Trio' },
      });
      expect((await t.api(alice).get(`/api/calls/${ids.live}`).expect(200)).body).toMatchObject({
        outcome: 'ongoing',
        call: { status: 'ringing' },
      });
      // Removed from my log (the previous test hid `declined` for alice): 404 for me, still there for bob.
      expect(
        (await t.api(alice).get(`/api/calls/${ids.declined}`).expect(404)).body.error.code,
      ).toBe('not_found');
      expect((await t.api(bob).get(`/api/calls/${ids.declined}`).expect(200)).body).toMatchObject({
        direction: 'outgoing',
        outcome: 'declined',
      });
      ...
      expect((await t.api(alice).get('/api/calls/not-a-uuid').expect(400)).body.error.code).toBe(
        'validation_error',
      );
      await t.api().get(`/api/calls/${ids.answered}`).expect(401);
      // The literal routes still win over the param route.
      expect(Array.isArray((await t.api(alice).get('/api/calls/active').expect(200)).body)).toBe(
        true,
      );
      expect((await t.api(alice).get('/api/calls/ice-servers').expect(200)).body).toHaveProperty(
        'iceServers',
      );
    });
    ...
    it('GET /calls/:callId: a callee who blocked the caller (hidden participant) gets 404', async () => {
      const [eve, frank] = await Promise.all([
        t.createUser({ displayName: 'Eve' }),
        t.createUser({ displayName: 'Frank' }),
      ]);
      ...
      expect((await t.api(frank).get(`/api/calls/${call.id}`).expect(200)).body).toMatchObject({
        direction: 'outgoing',
        call: { id: call.id },
      });
    ...
    it('DELETE /calls clears my ended calls (live calls stay)', async () => {
      ...
      expect(
        ((await t.api(alice).get('/api/calls').expect(200)).body as CallLogEntry[]).length,
      ).toBeGreaterThan(1);
    });
  });
  ...
  describe('GET /calls/active', () => {
    it('lists live calls of my active chats (ringing me, or joinable), not ended ones nor former chats', async () => {
      ...
      const grp = await ackCall(c!, 'call:start', {
        chatId: group,
        type: 'audio',
        userIds: [dave!.id],
      });
      ...
      expect(
        ((await t.api(dave!).get('/api/calls/active').expect(200)).body as Call[]).map((x) => x.id),
      ).toEqual([grp.id]);
      ...
      expect(
        ((await t.api(bob!).get('/api/calls/active').expect(200)).body as Call[]).map((x) => x.id),
      ).toEqual([direct.id]);
  ...
  describe('GET /calls/ice-servers', () => {
    ...
    it('uses static TURN credentials when no secret is configured', async () => {
      ...
      expect(res.body.iceServers).toEqual([
        { urls: ['turn:t.example.org'], username: 'user', credential: 'pass' },
      ]);
  ...
  it('rate-limits call:start per user (USER_RATE_LIMITS.callStart)', async () => {
    ...
    try {
      for (let i = 0; i < 10; i++)
        expect(
          (await ackError(s, 'call:start', { chatId: crypto.randomUUID(), type: 'audio' })).code,
        ).toBe('not_found');
      expect(
        (await ackError(s, 'call:start', { chatId: crypto.randomUUID(), type: 'audio' })).code,
      ).toBe('rate_limited');
