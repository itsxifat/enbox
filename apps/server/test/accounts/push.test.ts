...
import { CALL_RING_TIMEOUT_MS, MUTE_FOREVER_ISO, PUSH_MESSAGE_TTL_SEC, mentionToken, type PushPayload } from '@enbox/shared';
import { config } from '../../src/config.js';
import { db } from '../../src/db/index.js';
import { callParticipants, calls, chatMembers, pushSubscriptions } from '../../src/db/schema.js';
import { getPushSender, pushIdle, setPushSender, type PushOptions, type PushTarget } from '../../src/modules/push/sender.js';
...
import { block, createChannel, createDirect, createGroup, saveContact, send, setSettings } from '../services/fixtures.js';
...
describe('push: subscriptions and notifications', () => {
  ...
  async function subscribe(user: TestUser): Promise<string> {
    ...
    await t.api(user).post('/api/push/subscriptions').send({ endpoint, keys: { p256dh: 'BPublicKey', auth: 'authSecret' } }).expect(204);
    return endpoint;
  }
  ...
  describe('POST/DELETE /push/subscriptions', () => {
    it('stores the subscription bound to my user and session', async () => {
      ...
      expect(row).toMatchObject({ userId: u.id, sessionId: u.sessionId, p256dh: 'BPublicKey', auth: 'authSecret' });
    });
    ...
    it.each([
      ...
      const u = await t.createUser();
      const res = await t.api(u).post('/api/push/subscriptions').send({ endpoint, keys: { p256dh: 'p', auth: 'a' } }).expect(400);
    ...
    it('accepts the allow-listed push services and validates keys', async () => {
      ...
      for (const endpoint of ['https://updates.push.services.mozilla.com/wpush/v2/x', 'https://web.push.apple.com/abc', 'https://wns2-par02p.notify.windows.com/w/?token=x']) {
        await t.api(u).post('/api/push/subscriptions').send({ endpoint, keys: { p256dh: 'p', auth: 'a' } }).expect(204);
      }
      await t.api(u).post('/api/push/subscriptions').send({ endpoint: 'https://fcm.googleapis.com/fcm/send/k' }).expect(400);
      await t.api(u).post('/api/push/subscriptions').send({ endpoint: 'https://fcm.googleapis.com/fcm/send/k', keys: { p256dh: '', auth: 'a' } }).expect(400);
    });
    ...
    it('upserts by endpoint: subscribing again reassigns the row to the current user and session', async () => {
      ...
      await t.api(b).post('/api/push/subscriptions').send({ endpoint, keys: { p256dh: 'new', auth: 'new' } }).expect(204);
      const rows = await db.select().from(pushSubscriptions).where(eq(pushSubscriptions.endpoint, endpoint));
      expect(rows).toHaveLength(1);
      expect(rows[0]).toMatchObject({ userId: b.id, sessionId: b.sessionId, p256dh: 'new', auth: 'new' });
    });
  ...
  describe('message pushes', () => {
    ...
    it('direct: every subscription of the recipient (not the sender) gets the message push', async () => {
      ...
      await pushIdle();
      const expected: PushPayload = { type: 'message', title: 'Alice', body: 'hello bob', tag: `chat:${direct}`, url: `/chats/${direct}`, chatId: direct };
      expect(to(bobEp)).toEqual([{ endpoint: bobEp, payload: expected, opts: { ttl: PUSH_MESSAGE_TTL_SEC } }]);
    ...
    it("titles direct pushes with the name the recipient saved, and uses the sender's visible avatar as icon", async () => {
      ...
      await pushIdle();
      expect(to(bobEp)[0]!.payload).toMatchObject({ title: 'Saved Name', body: 'yo', icon: avatarUrl });
    ...
    it('groups: title = group name, body = "Sender: preview", mentions rendered as each recipient knows the users', async () => {
      ...
      await pushIdle();
      expect(to(bobEp)[0]!.payload).toMatchObject({ type: 'message', title: 'Friends', body: 'Alice: hey @Bob and @Carol', tag: `chat:${group}`, url: `/chats/${group}`, chatId: group });
    ...
    it('muted chats never push (until the mute expires)', async () => {
      await db.update(chatMembers).set({ mutedUntil: new Date(MUTE_FOREVER_ISO) }).where(and(eq(chatMembers.chatId, direct), eq(chatMembers.userId, bob.id)));
      await send(alice, direct, 'muted');
      await pushIdle();
      expect(sent).toEqual([]);
      await db.update(chatMembers).set({ mutedUntil: new Date(Date.now() - 1000) }).where(and(eq(chatMembers.chatId, direct), eq(chatMembers.userId, bob.id)));
    ...
    it('never for channels, call messages or messages withheld by a block', async () => {
      ...
      await transact((tx, fx) => upsertMembership(tx, fx, { kind: 'activate', chatId: channel, userIds: [bob.id] }));
      await send(alice, channel, 'channel post');
      await send(alice, direct, {
        type: 'call',
        text: null,
        clientId: null,
        metadata: { call: { callId: crypto.randomUUID(), callType: 'audio', isGroup: false, initiatorId: alice.id, status: 'ringing', durationSec: null } },
      });
    ...
    it('renders non-text previews and truncates long ones to 120 characters', async () => {
      await send(alice, direct, { type: 'location', text: null, metadata: { location: { latitude: 1, longitude: 2, name: 'Home', address: null } } });
    ...
    it('deletes subscriptions the push service reports gone (404/410) and keeps them on other errors', async () => {
      ...
      await pushIdle();
      expect(await db.select().from(pushSubscriptions).where(eq(pushSubscriptions.endpoint, bobEp))).toEqual([]);
      expect(await db.select().from(pushSubscriptions).where(eq(pushSubscriptions.endpoint, bobEp2))).toHaveLength(1);
      failWith.clear();
      failWith.set(bobEp2, 404);
      await send(alice, direct, 'second');
      await pushIdle();
      expect(await db.select().from(pushSubscriptions).where(eq(pushSubscriptions.userId, bob.id))).toEqual([]);
      expect(sent).toEqual([]);
    });
    ...
    it('dismiss: reading clears the notification on my devices (only when unread messages were cleared)', async () => {
      ...
      await pushIdle();
      sent = [];
      await transact((tx, fx) => advanceRead(tx, fx, { chatId: direct, userId: bob.id, seq: 1_000_000 }));
      await pushIdle();
      const dismiss: PushPayload = { type: 'dismiss', title: '', body: '', tag: `chat:${direct}`, url: `/chats/${direct}`, chatId: direct };
    ...
    it('dismiss is never sent for channels', async () => {
      ...
      await transact((tx, fx) => advanceRead(tx, fx, { chatId: channel, userId: bob.id, seq: 1_000_000 }));
      await pushIdle();
      expect(sent).toEqual([]);
    });
  });
  ...
  describe('call pushes', () => {
    ...
    async function insertCall(opts: { hidden?: boolean; isGroup?: boolean; chat?: string } = {}) {
      const [call] = await db
        .insert(calls)
        .values({ chatId: opts.chat ?? chatId, initiatorId: caller.id, type: 'audio', isGroup: opts.isGroup ?? false, status: 'missed' })
        .returning();
      await db.insert(callParticipants).values([
        { callId: call!.id, userId: caller.id, status: 'left' },
        { callId: call!.id, userId: callee.id, status: 'missed', hiddenAt: opts.hidden ? new Date() : null },
      ]);
      return call!.id;
    }
    ...
    it('call.ringing → a high-urgency call push with the ring timeout as TTL (not to silent callees or with callNotifications off)', async () => {
      ...
      domainEvents.emit('call.ringing', { callId, chatId, callerId: caller.id, callType: 'video', isGroup: false, userIds: [callee.id, silent.id, muted.id], silentUserIds: [silent.id] });
      await pushIdle();
      expect(sent).toEqual([
        {
          endpoint: calleeEp,
          payload: { type: 'call', title: 'Caller', body: 'Incoming video call', tag: `call:${callId}`, url: `/chats/${chatId}`, chatId, callId },
    ...
    it('group calls are titled with the group name', async () => {
      ...
      const callId = crypto.randomUUID();
      domainEvents.emit('call.ringing', { callId, chatId: group, callerId: caller.id, callType: 'audio', isGroup: true, userIds: [callee.id], silentUserIds: [] });
      await pushIdle();
      expect(to(calleeEp).map((s) => s.payload)).toEqual([
        { type: 'call', title: 'Team', body: 'Caller · Incoming group voice call', tag: `call:${callId}`, url: `/chats/${group}`, chatId: group, callId },
      ]);
    });

    it('call.ring-stopped → call_cancel ("Missed call" only when missed)', async () => {
      const callId = await insertCall();
      domainEvents.emit('call.ring-stopped', { callId, chatId, userId: callee.id, reason: 'timeout', finalStatus: 'missed' });
      await pushIdle();
      expect(to(calleeEp)).toEqual([
        {
          endpoint: calleeEp,
          payload: { type: 'call_cancel', title: 'Caller', body: 'Missed call', tag: `call:${callId}`, url: `/chats/${chatId}`, chatId, callId },
          opts: { ttl: PUSH_MESSAGE_TTL_SEC, urgency: 'high' },
        },
      ]);
      sent = [];
      domainEvents.emit('call.ring-stopped', { callId, chatId, userId: callee.id, reason: 'answered_elsewhere', finalStatus: 'joined' });
      await pushIdle();
    ...
    it('no call_cancel for silent rings, hidden participants or callNotifications off', async () => {
      ...
      domainEvents.emit('call.ring-stopped', { callId: silentCall, chatId, userId: callee.id, reason: 'timeout', finalStatus: 'missed' });
      await pushIdle();
      await setSettings(callee, { silenceUnknownCallers: false });
      const hiddenCall = await insertCall({ hidden: true });
      domainEvents.emit('call.ring-stopped', { callId: hiddenCall, chatId, userId: callee.id, reason: 'timeout', finalStatus: 'missed' });
      await pushIdle();
      await setSettings(callee, { callNotifications: false });
      const offCall = await insertCall();
      domainEvents.emit('call.ring-stopped', { callId: offCall, chatId, userId: callee.id, reason: 'timeout', finalStatus: 'missed' });
      await pushIdle();
      expect(sent).toEqual([]);
      ...
      domainEvents.emit('call.ring-stopped', { callId: knownCall, chatId, userId: callee.id, reason: 'cancelled', finalStatus: 'missed' });
      await pushIdle();
