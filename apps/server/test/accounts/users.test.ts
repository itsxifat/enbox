...
describe('users: profiles, lookups, search, common groups', () => {
  ...
  const getUser = async (as: TestUser, id: string) =>
    (await t.api(as).get(`/api/users/${id}`).expect(200)).body as UserPublic;
  const search = async (as: TestUser, q: string) =>
    ((await t.api(as).get('/api/users/search').query({ q }).expect(200)).body as UserPublic[]).map(
      (u) => u.id,
    );
  ...
  describe('GET /users/:userId privacy', () => {
    ...
    it("'contacts' levels depend on whether the SUBJECT saved me", async () => {
      ...
      expect(await getUser(viewer, u.id)).toMatchObject({
        avatarUrl: null,
        about: null,
        phone: null,
        online: null,
    ...
    it("'nobody' hides everything even from contacts; I always see myself", async () => {
      ...
      expect(await getUser(u, u.id)).toMatchObject({
        avatarUrl,
        about: 'hello there',
        online: false,
      });
    ...
    it('deleted accounts render as "Deleted account"; unknown ids 404; malformed ids 400', async () => {
      ...
      expect(await getUser(viewer, gone.id)).toMatchObject({
  ...
  describe('lookups', () => {
    it('GET /users/by-username/:username finds active users (case-insensitive) but not deleted users or users who blocked me', async () => {
      ...
      expect(
        (await t.api(viewer).get('/api/users/by-username/LOOKUP_ME').expect(200)).body.id,
      ).toBe(u.id);
    ...
    it('POST /users/batch: input order, unknown ids omitted, deleted users included, limits validated', async () => {
      ...
      const res = await t
        ...
        .send({ userIds: [b.id, crypto.randomUUID(), gone.id, a.id.toUpperCase()] })
        .expect(200);
  ...
  describe('GET /users/search', () => {
    ...
    beforeAll(async () => {
      ...
      target = await t.createUser({
        username: 'zebra_stripes',
        displayName: 'Marty Zebra',
        phone: targetPhone,
      });
    });
    ...
    it('matches an exact username (leading @, any case), a ≥3-char username prefix and an exact phone', async () => {
      ...
      expect(await search(me, `${targetPhone.slice(0, 5)} ${targetPhone.slice(5)}`)).toEqual([
        target.id,
      ]);
    ...
    it('matches display names (and my saved names) among contacts and active chat partners only', async () => {
      ...
      await transact((tx, fx) =>
        upsertMembership(tx, fx, {
          ...
          userId: former.id,
          reason: 'left',
          systemEvent: { kind: 'member_left', actorId: former.id },
        }),
      );
      const channelId = await createChannel(me);
      await transact((tx, fx) =>
        upsertMembership(tx, fx, { kind: 'activate', chatId: channelId, userIds: [follower.id] }),
      );
    ...
    it('ranks exact matches first, treats LIKE wildcards literally and caps results at USER_SEARCH_LIMIT', async () => {
      for (let i = 0; i < USER_SEARCH_LIMIT + 5; i++)
        await t.createUser({ username: `many_x${String(i).padStart(2, '0')}` });
    ...
    it('is rate-limited per user (shared with adding contacts)', async () => {
      ...
      try {
        resetUserLimits();
        for (let i = 0; i < USER_RATE_LIMITS.userSearch.limit; i++)
          await t.api(u).get('/api/users/search').query({ q: 'abc' }).expect(200);
        expect(
          (await t.api(u).get('/api/users/search').query({ q: 'abc' }).expect(429)).body.error.code,
        ).toBe('rate_limited');
    ...
    it('lists active regular groups we share (not channels, announcement groups, left groups)', async () => {
      ...
      await transact((tx, fx) =>
        upsertMembership(tx, fx, {
          ...
          userId: b.id,
          reason: 'left',
          systemEvent: { kind: 'member_left', actorId: b.id },
        }),
      );
      const channel = await createChannel(a);
      await transact((tx, fx) =>
        upsertMembership(tx, fx, { kind: 'activate', chatId: channel, userIds: [b.id] }),
      );
      const { announcementChatId } = await createCommunity(a);
      await transact((tx, fx) =>
        upsertMembership(tx, fx, { kind: 'activate', chatId: announcementChatId, userIds: [b.id] }),
      );

      const list = (await t.api(a).get(`/api/users/${b.id}/common-groups`).expect(200))
        .body as ChatSummary[];
      ...
      const fromB = (await t.api(b).get(`/api/users/${a.id}/common-groups`).expect(200))
        .body as ChatSummary[];
      ...
      expect(
        (await t.api(a).get(`/api/users/${viewer.id}/common-groups`).expect(200)).body,
      ).toEqual([]);
