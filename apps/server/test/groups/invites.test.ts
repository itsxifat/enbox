...
import { MAX_GROUP_MEMBERS, type Community, type InviteJoinResult, type InvitePreview } from '@enbox/shared';
...
import { bulkCommunityJoin, bulkJoin, bulkUsers, connectAll, expectError, summary, systemKinds } from './util.js';
...
describe('invites module', () => {
  ...
  const groupCode = async (chatId: string) => (await api(owner).get(`/api/groups/${chatId}/invite`).expect(200)).body.code as string;
  const preview = async (u: TestUser, code: string) => (await api(u).get(`/api/invites/${code}`).expect(200)).body as InvitePreview;
  ...
  it('invite lookups and joins are rate-limited per IP', async () => {
    ...
    try {
      let last = 0;
      for (let i = 0; i < 61; i++) last = (await api(u).get(`/api/invites/${'B'.repeat(22)}`)).status;
  ...
  describe('groups', () => {
    it('preview: members vs outsiders', async () => {
      ...
      expect(await preview(member!, code)).toMatchObject({ isMember: true, canJoin: false, reason: null });
    });
    ...
    it('join: JOIN(me) then member_joined_via_link; room gets memberCount + members-changed; already a member → 409', async () => {
      ...
      expect(body.chat).toMatchObject({ id: chatId, membership: 'active', myRole: 'member', memberCount: 3 });
      await settle();
      const j = jRec!.log.filter((e) => ['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed'].includes(e.event));
      expect(j.map((e) => e.event)).toEqual(['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed']);
      expect(j[1]!.payload.message.system).toEqual({ kind: 'member_joined_via_link', actorId: joiner!.id });
      expect(mRec!.names().filter((n) => ['message:new', 'chat:updated', 'chat:members-changed'].includes(n))).toEqual(['message:new', 'chat:updated', 'chat:members-changed']);
      const page = await loadMessagePage(db, joiner!.id, chatId, { limit: 50 });
      expect(page.messages.map((m) => m.system?.kind ?? m.text)).toEqual(['member_joined_via_link']);
    ...
    it('former members: left → may rejoin; removed by an admin → canJoin false + 403', async () => {
      ...
      expect(await preview(leaver!, code)).toMatchObject({ isMember: false, canJoin: true, reason: null });
      expect(await preview(kicked!, code)).toMatchObject({ isMember: false, canJoin: false, reason: 'You were removed by an admin' });
    ...
    it('full groups: preview reason + 409 limit_reached', async () => {
      ...
      const code = await groupCode(chatId);
      expect(await preview(joiner, code)).toMatchObject({ canJoin: false, reason: 'This group is full', memberCount: MAX_GROUP_MEMBERS });
    ...
    it('joining an emptied group makes the joiner its owner', async () => {
      ...
      expect((await systemKinds(res.body.chat.id)).slice(-2)).toEqual(['member_joined_via_link', 'owner_changed']);
    });
    ...
    it('linked groups: preview shows the community; joining also joins the community (cascade)', async () => {
      ...
      const community = (await api(owner).post('/api/communities').send({ name: 'Hood', groupIds: [g] }).expect(201)).body as Community;
      const code = await groupCode(g);
      expect(await preview(joiner!, code)).toMatchObject({ kind: 'group', communityId: community.id, communityName: 'Hood', canJoin: true });
      ...
      await settle();
      const names = rec.log
        .filter((e) => ['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed', 'community:upsert'].includes(e.event))
        .map((e) => (e.event === 'chat:upsert' || e.event === 'chat:updated' ? `${e.event}:${(e.payload.chat?.id ?? e.payload.chatId) === g ? 'g' : 'ann'}` : e.event));
      expect(names).toEqual(['chat:upsert:g', 'message:new', 'chat:updated:g', 'chat:members-changed', 'chat:upsert:ann', 'chat:updated:ann', 'community:upsert']);
      ...
      await bulkCommunityJoin(community.id, community.announcementChatId, await bulkUsers(MAX_GROUP_MEMBERS - 2));
      expect(await preview(late!, code)).toMatchObject({ canJoin: false, reason: 'This community is full' });
  ...
  describe('channels', () => {
    it('private channels are joined through the link (as a follow); already following → 409', async () => {
      ...
      const ch = (await api(owner).post('/api/channels').send({ name: 'Insiders', isPublic: false }).expect(201)).body;
      const code = (await api(owner).get(`/api/channels/${ch.id}/invite`).expect(200)).body.code as string;
      ...
      expect(res.chat).toMatchObject({ type: 'channel', membership: 'active', unreadCount: 0, memberCount: 2 });
      await settle();
      expect(jRec!.names().filter((n) => n !== 'presence:update')).toEqual(['chat:upsert', 'chat:updated']);
      expect(oRec!.names().filter((n) => ['chat:updated', 'chat:members-changed'].includes(n))).toEqual(['chat:updated', 'chat:members-changed']);
      expect((await loadMessagePage(db, joiner!.id, ch.id, { limit: 10 })).messages.map((m) => m.system?.kind ?? m.text)).toEqual(['channel_created', 'post 1']);
  ...
  describe('communities', () => {
    it('preview + join into the announcement group; removed members get 403; already a member → 409', async () => {
      ...
      const c = (await api(owner).post('/api/communities').send({ name: 'Town', description: 'Our town' }).expect(201)).body as Community;
      ...
      expect(res).toMatchObject({ kind: 'community', id: c.id, chat: null, community: { id: c.id, myRole: 'member', memberCount: 2 } });
      await settle();
      expect(jRec!.names().filter((n) => ['chat:upsert', 'community:upsert', 'chat:updated'].includes(n))).toEqual(['chat:upsert', 'community:upsert', 'chat:updated']);
      expect(jRec!.of('chat:upsert')[0]!.chat).toMatchObject({ id: c.announcementChatId, isAnnouncement: true });
      ...
      expect(await preview(joiner!, c.inviteCode!)).toMatchObject({ isMember: true, canJoin: false });
      expectError(await join(joiner!, c.inviteCode!), 409, 'conflict');

      await api(owner).post(`/api/communities/${c.id}/members`).send({ userIds: [kicked!.id] }).expect(200);
      await api(owner).delete(`/api/communities/${c.id}/members/${kicked!.id}`).expect(204);
      expect(await preview(kicked!, c.inviteCode!)).toMatchObject({ canJoin: false, reason: 'You were removed by an admin' });
    ...
    it('full communities: preview reason + 409', async () => {
      const joiner = await t.createUser();
      const c = (await api(owner).post('/api/communities').send({ name: 'Packed' }).expect(201)).body as Community;
      await bulkCommunityJoin(c.id, c.announcementChatId, await bulkUsers(MAX_GROUP_MEMBERS - 1));
      expect(await preview(joiner, c.inviteCode!)).toMatchObject({ canJoin: false, reason: 'This community is full' });
