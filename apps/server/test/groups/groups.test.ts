...
import { DEFAULT_GROUP_SETTINGS, MAX_GROUP_MEMBERS, type AddMembersResult, type ChatSummary } from '@enbox/shared';
...
import { block, createChannel, createCommunity, createDirect, createGroup, memberRow, recordEvents, saveContact, send, setSettings, settle } from '../services/fixtures.js';
import { bulkJoin, bulkUsers, connectAll, expectError, summary, systemKinds, uploadImage } from './util.js';
...
describe('groups module', () => {
  ...
  describe('POST /groups', () => {
    it('creates the group, adds eligible members and reports needsInvite/failed', async () => {
      ...
      const res = await api(owner)
        ...
        .send({
          ...
          memberIds: [free!.id, blocker!.id, blockedByMe!.id, nobody!.id, contactsOnly!.id, saved!.id, deleted!.id, unknown, owner.id],
          settings: { onlyAdminsCanSend: true },
        })
        .expect(201);
      const body = res.body as AddMembersResult;
      expect(body.added).toEqual([free!.id, saved!.id]);
      expect(body.needsInvite).toEqual([blocker!.id, blockedByMe!.id, nobody!.id, contactsOnly!.id]);
    ...
    it('emits JOIN (chat:upsert) before the system messages to every initial member; nothing to needsInvite users', async () => {
      ...
      const res = await api(owner).post('/api/groups').send({ name: 'G', memberIds: [a!.id, b!.id] }).expect(201);
      ...
      for (const rec of [ownerRec, aRec]) {
        ...
        expect(relevant.slice(1).map((e) => e.payload.message.system.kind)).toEqual(['group_created', 'members_added']);
    ...
    it("uses the creator's default disappearing timer unless given; validates avatars and bodies", async () => {
      ...
      const r2 = await api(creator).post('/api/groups').send({ name: 'No timer', disappearingSeconds: null }).expect(201);
      expect(r2.body.chat.disappearingSeconds).toBeNull();

      const mine = await uploadImage(t, creator);
      const r3 = await api(creator).post('/api/groups').send({ name: 'Pic', avatarMediaId: mine }).expect(201);
      expect(r3.body.chat.avatarUrl).toMatch(/^\/uploads\/.+\.png$/);
      const theirs = await uploadImage(t, owner);
      expectError(await api(creator).post('/api/groups').send({ name: 'Pic', avatarMediaId: theirs }), 404, 'not_found');
      const doc = (await api(creator).post('/api/media').field('kind', 'file').attach('file', Buffer.from('plain text'), { filename: 'a.txt' }).expect(201)).body.id;
      expectError(await api(creator).post('/api/groups').send({ name: 'Doc', avatarMediaId: doc }), 400, 'validation_error');

      expectError(await api(creator).post('/api/groups').send({ name: '   ' }), 400, 'validation_error');
      const x = crypto.randomUUID();
      expectError(await api(creator).post('/api/groups').send({ name: 'Dup', memberIds: [x, x] }), 400, 'validation_error');
      expectError(await api(creator).post('/api/groups').send({ name: 'Bad', disappearingSeconds: 5 }), 400, 'validation_error');
  ...
  describe('route guards', () => {
    it('404 for outsiders and non-groups, 403 for announcement groups, 403 not_member for former members', async () => {
      ...
      expectError(await api(outsider!).patch(`/api/groups/${chatId}`).send({ name: 'x' }), 404, 'not_found');
      expectError(await api(outsider!).get(`/api/groups/${chatId}/invite`), 404, 'not_found');
      expectError(await api(owner).patch(`/api/groups/${channelId}`).send({ name: 'x' }), 404, 'not_found');
      expectError(await api(owner).post(`/api/groups/${directId}/leave`), 404, 'not_found');
      expectError(await api(owner).patch(`/api/groups/${crypto.randomUUID()}`).send({ name: 'x' }), 404, 'not_found');
      expectError(await api(owner).patch('/api/groups/not-a-uuid').send({ name: 'x' }), 400, 'validation_error');
      ...
      expectError(await api(member!).patch(`/api/groups/${chatId}`).send({ name: 'x' }), 403, 'not_member');
      ...
      expectError(await api(member!).post(`/api/groups/${chatId}/members`).send({ userIds: [outsider!.id] }), 403, 'not_member');
    });
  });
  ...
  describe('PATCH /groups/:chatId', () => {
    it('one system message per changed field, then chat:updated with the changed fields', async () => {
      ...
      const res = await api(member).patch(`/api/groups/${chatId}`).send({ name: 'New', description: 'About', avatarMediaId: avatar }).expect(200);
      ...
      expect(relevant.map((e) => (e.event === 'message:new' ? e.payload.message.system.kind : e.event))).toEqual([
        'name_changed',
        'description_changed',
        'avatar_changed',
        'chat:updated',
      ]);
      expect(relevant[0]!.payload.message.system).toEqual({ kind: 'name_changed', actorId: member.id, name: 'New' });
      expect(relevant[3]!.payload).toEqual({ chatId, changes: { name: 'New', description: 'About', avatarUrl: res.body.avatarUrl } });

      rec.clear();
      await api(member).patch(`/api/groups/${chatId}`).send({ name: 'New', description: 'About' }).expect(200);
      await settle();
      expect(rec.log.filter((e) => ['message:new', 'chat:updated'].includes(e.event))).toEqual([]);

      await api(owner).patch(`/api/groups/${chatId}`).send({ description: null, avatarMediaId: null }).expect(200);
      await settle();
      expect(rec.of('chat:updated').at(-1)!.changes).toEqual({ description: null, avatarUrl: null });
      expect((await systemKinds(chatId)).slice(-2)).toEqual(['description_changed', 'avatar_changed']);
      await conns.close();
    });

    it('onlyAdminsCanEditInfo: members get 403, admins may edit', async () => {
      const [member, admin] = await makeUsers(2);
      const chatId = await createGroup(owner, [member!, admin!], { settings: { onlyAdminsCanEditInfo: true }, admins: [admin!] });
      expectError(await api(member!).patch(`/api/groups/${chatId}`).send({ name: 'Nope' }), 403, 'forbidden');
  ...
  describe('PATCH /groups/:chatId/settings', () => {
    it('admins only; one settings_changed per changed key, then chat:updated {groupSettings}', async () => {
      ...
      expectError(await api(member!).patch(`/api/groups/${chatId}/settings`).send({ onlyAdminsCanSend: true }), 403, 'forbidden');
      const conns = await connectAll(t, member!);
      const rec = recordEvents(conns.sockets[0]!);
      const res = await api(admin!)
        .patch(`/api/groups/${chatId}/settings`)
        .send({ onlyAdminsCanSend: true, onlyAdminsCanEditInfo: false, onlyAdminsCanAddMembers: true })
        .expect(200);
      expect(res.body.groupSettings).toEqual({ onlyAdminsCanSend: true, onlyAdminsCanEditInfo: false, onlyAdminsCanAddMembers: true });
      await settle();
      const relevant = rec.log.filter((e) => ['message:new', 'chat:updated'].includes(e.event));
      expect(relevant.map((e) => (e.event === 'message:new' ? e.payload.message.system : e.event))).toEqual([
        { kind: 'settings_changed', actorId: admin!.id, setting: 'onlyAdminsCanSend', value: true },
        { kind: 'settings_changed', actorId: admin!.id, setting: 'onlyAdminsCanAddMembers', value: true },
        'chat:updated',
      ]);
      ...
      expect(s.permissions).toMatchObject({ canSend: false, canAddMembers: false, canInvite: false, canEditInfo: true });
      expect(s.inviteCode).toBeNull();

      rec.clear();
      await api(owner).patch(`/api/groups/${chatId}/settings`).send({ onlyAdminsCanSend: true }).expect(200);
      await settle();
      expect(rec.log.filter((e) => ['message:new', 'chat:updated'].includes(e.event))).toEqual([]);
      expectError(await api(owner).patch(`/api/groups/${chatId}/settings`).send({ onlyAdminsCanSend: 'yes' }), 400, 'validation_error');
      await conns.close();
    });
  });
  ...
  describe('POST /groups/:chatId/members', () => {
    it('adds members: new member gets chat:upsert BEFORE message:new; room gets memberCount + members-changed', async () => {
      ...
      const res = await api(member!).post(`/api/groups/${chatId}/members`).send({ userIds: [newbie!.id, shy!.id, member!.id] }).expect(200);
      const body = res.body as AddMembersResult;
      ...
      await settle();
      const nb = newbieRec.log.filter((e) => ['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed'].includes(e.event));
      expect(nb.map((e) => e.event)).toEqual(['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed']);
      expect(nb[0]!.payload.chat).toMatchObject({ id: chatId, membership: 'active', memberCount: 3 });
      expect(nb[1]!.payload.message.system).toEqual({ kind: 'members_added', actorId: member!.id, userIds: [newbie!.id] });
      expect(nb[2]!.payload).toEqual({ chatId, changes: { memberCount: 3 } });
      const mb = memberRec.log.filter((e) => ['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed'].includes(e.event));
      expect(mb.map((e) => e.event)).toEqual(['message:new', 'chat:updated', 'chat:members-changed']);
    ...
    it('onlyAdminsCanAddMembers: members get 403; nothing eligible → no events', async () => {
      ...
      await setSettings(shy!, { groupsAddPermission: 'nobody' });
      const chatId = await createGroup(owner, [member!, admin!], { settings: { onlyAdminsCanAddMembers: true }, admins: [admin!] });
      expectError(await api(member!).post(`/api/groups/${chatId}/members`).send({ userIds: [target!.id] }), 403, 'forbidden');
      const conns = await connectAll(t, member!);
      const rec = recordEvents(conns.sockets[0]!);
      const res = await api(admin!).post(`/api/groups/${chatId}/members`).send({ userIds: [shy!.id] }).expect(200);
      expect(res.body).toMatchObject({ added: [], needsInvite: [shy!.id], failed: [] });
      await settle();
      expect(rec.log.filter((e) => e.event !== 'presence:update')).toEqual([]);
      expectError(await api(admin!).post(`/api/groups/${chatId}/members`).send({ userIds: [] }), 400, 'validation_error');
      await conns.close();
    });
    ...
    it('re-adding a removed member reactivates the row with a new window', async () => {
      ...
      await api(owner).post(`/api/groups/${chatId}/members`).send({ userIds: [member!.id] }).expect(200);
    ...
    it('capacity: users beyond MAX_GROUP_MEMBERS fail with limit_reached', async () => {
      ...
      const res = await api(owner).post(`/api/groups/${chatId}/members`).send({ userIds: [x!.id, y!.id] }).expect(200);
      expect(res.body).toMatchObject({ added: [x!.id], needsInvite: [], failed: [{ userId: y!.id, reason: 'limit_reached' }] });
    ...
    it('rate limit: USER_RATE_LIMITS.addMembers counts added users (a rejected request adds nobody)', async () => {
      ...
      try {
        const res = await api(adder).post('/api/groups').send({ name: 'Big', memberIds: ids.slice(0, 150) }).expect(201);
        const chatId = res.body.chat.id as string;
        expectError(await api(adder).post(`/api/groups/${chatId}/members`).send({ userIds: ids.slice(150, 210) }), 429, 'rate_limited');
        ...
        await api(adder).post(`/api/groups/${chatId}/members`).send({ userIds: ids.slice(100, 200) }).expect(200);
        expect((await summary(adder, chatId))!.memberCount).toBe(201);
        expectError(await api(adder).post(`/api/groups/${chatId}/members`).send({ userIds: [ids[200]!] }), 429, 'rate_limited');
      } finally {
  ...
  describe('DELETE /groups/:chatId/members/:userId', () => {
    it('removal: member_removed → R, then chat:upsert (removed) → U(u), then no more room events', async () => {
      ...
      await settle();
      const v = victimRec.log.filter((e) => ['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed'].includes(e.event));
      expect(v.map((e) => e.event)).toEqual(['message:new', 'chat:upsert']);
      expect(v[0]!.payload.message.system).toEqual({ kind: 'member_removed', actorId: admin!.id, userId: victim!.id });
      expect(v[1]!.payload.chat).toMatchObject({ id: chatId, membership: 'removed', myRole: 'member', memberCount: 3 });
      expect(v[1]!.payload.chat.permissions.canSend).toBe(false);
      const o = otherRec.log.filter((e) => ['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed'].includes(e.event));
      expect(o.map((e) => e.event)).toEqual(['message:new', 'chat:updated', 'chat:members-changed']);
      ...
      await settle();
      expect(victimRec.log.filter((e) => ['message:new', 'chat:updated', 'chat:members-changed'].includes(e.event))).toEqual([]);
    ...
    it('permissions: members 403; nobody removes the owner; admins may remove admins; 404 non-members; 400 self', async () => {
      ...
      const chatId = await createGroup(owner, [member!, admin1!, admin2!], { admins: [admin1!, admin2!] });
      expectError(await api(member!).delete(`/api/groups/${chatId}/members/${admin1!.id}`), 403, 'forbidden');
      expectError(await api(admin1!).delete(`/api/groups/${chatId}/members/${owner.id}`), 403, 'forbidden');
      expectError(await api(admin1!).delete(`/api/groups/${chatId}/members/${outsider!.id}`), 404, 'not_found');
      expectError(await api(admin1!).delete(`/api/groups/${chatId}/members/${admin1!.id}`), 400, 'validation_error');
      await api(admin1!).delete(`/api/groups/${chatId}/members/${admin2!.id}`).expect(204);
      expectError(await api(admin1!).delete(`/api/groups/${chatId}/members/${admin2!.id}`), 404, 'not_found');
      expect(await memberRow(chatId, admin2!)).toMatchObject({ role: 'member', leftReason: 'removed' });
    });
  });
  ...
  describe('roles and ownership', () => {
    it('promote/demote: system message, chat:upsert → target, members-changed; owner cannot be demoted', async () => {
      ...
      expectError(await api(member!).put(`/api/groups/${chatId}/members/${target!.id}/role`).send({ role: 'admin' }), 403, 'forbidden');
      const conns = await connectAll(t, target!);
      const rec = recordEvents(conns.sockets[0]!);
      await api(admin!).put(`/api/groups/${chatId}/members/${target!.id}/role`).send({ role: 'admin' }).expect(204);
      await settle();
      const r = rec.log.filter((e) => ['chat:upsert', 'message:new', 'chat:members-changed'].includes(e.event));
      expect(r.map((e) => e.event)).toEqual(['message:new', 'chat:upsert', 'chat:members-changed']);
      expect(r[0]!.payload.message.system).toEqual({ kind: 'admin_promoted', actorId: admin!.id, userId: target!.id });
      expect(r[1]!.payload.chat).toMatchObject({ myRole: 'admin', permissions: { canRemoveMembers: true, canManageAdmins: true } });

      rec.clear();
      await api(admin!).put(`/api/groups/${chatId}/members/${target!.id}/role`).send({ role: 'admin' }).expect(204); // unchanged
      await settle();
      expect(rec.log.filter((e) => e.event !== 'presence:update')).toEqual([]);
      await api(owner).put(`/api/groups/${chatId}/members/${target!.id}/role`).send({ role: 'member' }).expect(204);
      expect((await systemKinds(chatId)).at(-1)).toBe('admin_demoted');
      expectError(await api(admin!).put(`/api/groups/${chatId}/members/${owner.id}/role`).send({ role: 'member' }), 403, 'forbidden');
      expectError(await api(admin!).put(`/api/groups/${chatId}/members/${crypto.randomUUID()}/role`).send({ role: 'admin' }), 404, 'not_found');
      expectError(await api(admin!).put(`/api/groups/${chatId}/members/${target!.id}/role`).send({ role: 'owner' }), 400, 'validation_error');
      await conns.close();
    });
    ...
    it('transfer-ownership: owner only; old owner becomes admin; owner_transferred + upserts to both', async () => {
      ...
      expectError(await api(admin!).post(`/api/groups/${chatId}/transfer-ownership`).send({ userId: member!.id }), 403, 'forbidden');
      expectError(await api(owner).post(`/api/groups/${chatId}/transfer-ownership`).send({ userId: outsider!.id }), 404, 'not_found');
      expectError(await api(owner).post(`/api/groups/${chatId}/transfer-ownership`).send({ userId: owner.id }), 400, 'validation_error');
      const conns = await connectAll(t, owner, member!);
      const [ownerRec, memberRec] = conns.sockets.map((s) => recordEvents(s));
      await api(owner).post(`/api/groups/${chatId}/transfer-ownership`).send({ userId: member!.id }).expect(204);
      await settle();
      expect(memberRec!.names().filter((n) => ['message:new', 'chat:upsert', 'chat:members-changed'].includes(n))).toEqual(['message:new', 'chat:upsert', 'chat:members-changed']);
      expect(memberRec!.of('message:new')[0]!.message.system).toEqual({ kind: 'owner_transferred', actorId: owner.id, userId: member!.id });
    ...
    it('leave: member_left (+ owner_changed on succession), LEAVE(me), chat:upsert → new owner, memberCount, members-changed', async () => {
      ...
      await settle();
      const o = ownerRec!.log.filter((e) => ['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed'].includes(e.event));
      ...
      const a = adminRec!.log.filter((e) => ['chat:upsert', 'message:new', 'chat:updated', 'chat:members-changed'].includes(e.event));
      expect(a.map((e) => (e.event === 'message:new' ? e.payload.message.system.kind : e.event))).toEqual([
        'member_left',
        'owner_changed',
        'chat:upsert',
        'chat:updated',
      ...
      expectError(await api(owner).post(`/api/groups/${chatId}/members`).send({ userIds: [member!.id] }), 403, 'not_member');
      await conns.close();
    });
    ...
    it('the last member leaving leaves an empty group (no owner)', async () => {
      ...
      const rows = await db.select().from(chatMembers).where(and(eq(chatMembers.chatId, res.body.chat.id)));
  ...
  describe('invite links', () => {
    it('GET invite requires canInvite; reset (admins) → new code, invite_link_reset, chat:upsert to inviters', async () => {
      ...
      const { body: reset } = await api(admin!).post(`/api/groups/${chatId}/invite/reset`).expect(200);
      ...
      for (const rec of [memberRec!, adminRec!]) {
        ...
        expect(r[0]!.payload.message.system).toEqual({ kind: 'invite_link_reset', actorId: admin!.id });
        expect(r[1]!.payload.chat.inviteCode).toBe(reset.code);
      }
      expectError(await api(member!).get(`/api/invites/${first.code}`), 404, 'not_found');
      expect((await api(member!).get(`/api/invites/${reset.code}`).expect(200)).body.id).toBe(chatId);

      // Adding restricted to admins: members can no longer see the link and get no upsert on reset.
      await api(owner).patch(`/api/groups/${chatId}/settings`).send({ onlyAdminsCanAddMembers: true }).expect(200);
  ...
  it('a group without an invite code gets one generated on first GET (stable afterwards)', async () => {
    ...
    const first = (await api(owner).get(`/api/groups/${chatId}/invite`).expect(200)).body.code as string;
    expect(first).toMatch(/^[A-Za-z0-9]{22}$/);
    expect((await api(owner).get(`/api/groups/${chatId}/invite`).expect(200)).body.code).toBe(first);
  ...
  it('chat summaries stay consistent with computeChatPermissions for every role', async () => {
    const [admin, member] = await makeUsers(2);
    const chatId = await createGroup(owner, [admin!, member!], { admins: [admin!], settings: { onlyAdminsCanSend: true } });
    ...
    expect(o.permissions).toMatchObject({ canSend: true, canManageAdmins: true, canRemoveMembers: true, canLeave: true });
    expect(a.permissions).toMatchObject({ canSend: true, canManageAdmins: true, canRemoveMembers: true });
    expect(m.permissions).toMatchObject({ canSend: false, canManageAdmins: false, canRemoveMembers: false, canInvite: true });
  });
});
