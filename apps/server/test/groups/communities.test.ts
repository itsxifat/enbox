...
describe('communities module', () => {
  ...
  async function createCommunity(
    ...
    return (await api(by).post('/api/communities').send({ name, groupIds }).expect(201))
      .body as Community;
  }
  ...
  describe('POST /communities', () => {
    it('creates the community + announcement group and links the initial groups (cascading their members)', async () => {
      ...
      expect(ann.permissions).toMatchObject({
        canSend: true,
        canEditInfo: false,
        canAddMembers: false,
        canLeave: false,
      ...
      expect(ann.lastMessage?.system).toEqual({
        kind: 'community_created',
        actorId: owner.id,
        name: 'Neighbours',
      });
      const annM1 = (await summary(m1!, c.announcementChatId))!;
      expect(annM1).toMatchObject({
        myRole: 'member',
        membership: 'active',
        permissions: { canSend: false, canViewMembers: false },
      });
      expect(await systemKinds(g1)).toEqual([
        'group_created',
        'members_added',
        'added_to_community',
      ]);
      ...
      const o = ownerRec!.log.filter((e) =>
        ['chat:upsert', 'message:new', 'chat:updated', 'community:upsert'].includes(e.event),
      );
      ...
      expect(m.map((e) => e.event)).toEqual([
        'message:new',
        'chat:updated',
        'chat:upsert',
        'chat:updated',
      ...
      expect(m[4]!.payload.community).toMatchObject({
        id: c.id,
        myRole: 'member',
        inviteCode: null,
      });
      expect(ownerRec!.of('chat:members-changed').map((p) => p.chatId)).toContain(
        c.announcementChatId,
      );
      await conns.close();
    });
    ...
    it('linking requires admin rights on each group and an unlinked regular group', async () => {
      ...
      expectError(
        await api(owner)
          .post('/api/communities')
          .send({ name: 'X', groupIds: [other.announcementChatId] }),
        409,
        'conflict',
      );
      expectError(
        await api(owner)
          .post('/api/communities')
          .send({ name: 'X', groupIds: [crypto.randomUUID()] }),
        404,
        'not_found',
      );
      expectError(
        await api(owner).post('/api/communities').send({ name: '' }),
        400,
        'validation_error',
      );
  ...
  describe('GET /communities[/:id]', () => {
    it('lists my communities; 404 for non-members; invite code only for owner/admins', async () => {
      ...
      expect((await api(owner).get(`/api/communities/${c.id}`).expect(200)).body.inviteCode).toBe(
        c.inviteCode,
      );
  ...
  describe('route permissions', () => {
    it('outsiders and former members get 404 everywhere; plain members get 403 on admin/owner routes', async () => {
      ...
      await api(owner)
        ...
        .send({ userIds: [leaver!.id, target!.id] })
        .expect(200);
      ...
      expectError(
        await api(owner).get(`/api/communities/${crypto.randomUUID()}`),
        404,
        'not_found',
      );
  ...
  describe('PATCH /communities/:id', () => {
    it('owner/admins only; mirrors into the announcement group with per-field messages; community:upsert to members', async () => {
      ...
      expectError(
        await api(member!).patch(`/api/communities/${c.id}`).send({ name: 'Nope' }),
        403,
        'forbidden',
      );
      ...
      const res = await api(owner)
        .patch(`/api/communities/${c.id}`)
        .send({ name: 'After', description: 'Street news' })
        .expect(200);
      expect(res.body).toMatchObject({ name: 'After', description: 'Street news' });
      await settle();
      const r = rec.log.filter((e) =>
        ['message:new', 'chat:updated', 'community:upsert'].includes(e.event),
      );
      expect(
        r.map((e) => (e.event === 'message:new' ? e.payload.message.system.kind : e.event)),
      ).toEqual(['name_changed', 'description_changed', 'chat:updated', 'community:upsert']);
      expect(r[0]!.payload.message.chatId).toBe(c.announcementChatId);
      expect(r[2]!.payload).toEqual({
        chatId: c.announcementChatId,
        changes: { name: 'After', description: 'Street news' },
      });
      ...
      expectError(
        await api(owner)
          .patch(`/api/communities/${c.id}`)
          .send({ avatarMediaId: await uploadImage(t, member!) }),
        404,
        'not_found',
      );
      await conns.close();
  ...
  describe('groups inside a community', () => {
    it('POST /communities/:id/groups creates a linked group; its members join the community', async () => {
      ...
      await api(owner)
        .post(`/api/communities/${c.id}/members`)
        .send({ userIds: [admin!.id, member!.id] })
        .expect(200);
      await api(owner)
        .put(`/api/communities/${c.id}/members/${admin!.id}/role`)
        .send({ role: 'admin' })
        .expect(204);
      expectError(
        await api(member!).post(`/api/communities/${c.id}/groups`).send({ name: 'Nope' }),
        403,
        'forbidden',
      );
      expectError(
        await api(outsider!).post(`/api/communities/${c.id}/groups`).send({ name: 'Nope' }),
        404,
        'not_found',
      );
      ...
      const res = await api(admin!)
        ...
        .send({ name: 'Linked', memberIds: [outsider!.id] })
        .expect(201);
      expect(res.body.added).toEqual([outsider!.id]);
      expect(res.body.chat).toMatchObject({
        communityId: c.id,
        myRole: 'owner',
        name: 'Linked',
      ...
      await settle();
      const o = outRec!.log.filter((e) =>
        ['chat:upsert', 'message:new', 'chat:updated', 'community:upsert'].includes(e.event),
      );
      expect(
        o.map((e) =>
          e.event === 'chat:upsert'
            ? `upsert:${e.payload.chat.id === groupId ? 'g' : 'ann'}`
            : e.event,
        ),
      ).toEqual([
        'upsert:g',
      ...
      expect(upserted.groups.find((g) => g.chatId === groupId)).toMatchObject({
        isMember: true,
        memberCount: 2,
      });
    ...
    it('link / unlink: system messages, communityId changes, cascade on link; nobody leaves on unlink', async () => {
      ...
      expectError(
        await api(member!)
          .post(`/api/communities/${c.id}/groups/link`)
          .send({ chatIds: [g] }),
        403,
        'forbidden',
      );
      ...
      const rec = recordEvents(conns.sockets[0]!);
      expectError(
        await api(member!).delete(`/api/communities/${c.id}/groups/${g}`),
        403,
        'forbidden',
      );
      expectError(
        await api(owner).delete(`/api/communities/${c.id}/groups/${c.announcementChatId}`),
        403,
        'forbidden',
      );
      expectError(
        await api(owner).delete(`/api/communities/${c.id}/groups/${crypto.randomUUID()}`),
        404,
        'not_found',
      );
      const after = (await api(owner).delete(`/api/communities/${c.id}/groups/${g}`).expect(200))
        .body as Community;
    ...
    it('POST /communities/:id/groups/:c/join: community members join linked groups (not if removed by an admin)', async () => {
      ...
      await api(owner)
        ...
        .expect(200);
      expectError(
        await api(outsider!).post(`/api/communities/${c.id}/groups/${g}/join`),
        404,
        'not_found',
      );
      ...
      expect(r.map((e) => e.event)).toEqual([
        'chat:upsert',
        'message:new',
        'chat:updated',
        'chat:members-changed',
      ...
      expect(
        (r[4]!.payload.community as Community).groups.find((x) => x.chatId === g)!.isMember,
      ).toBe(true);
      expectError(
        await api(member!).post(`/api/communities/${c.id}/groups/${g}/join`),
        409,
        'conflict',
      );
      expectError(
        await api(member!).post(`/api/communities/${c.id}/groups/${c.announcementChatId}/join`),
        409,
        'conflict',
      );
      ...
      expectError(
        await api(kicked!).post(`/api/communities/${c.id}/groups/${g}/join`),
        403,
        'forbidden',
      );
    ...
    it('invariant: adding a non-member to a linked group makes them a community member', async () => {
      ...
      const names = rec.log
        ...
        .map((e) =>
          e.event === 'chat:upsert' ? `upsert:${e.payload.chat.id === g ? 'g' : 'ann'}` : e.event,
        );
    ...
    it('limits: MAX_COMMUNITY_GROUPS linked groups; community capacity on cascades', async () => {
      ...
      expectError(
        await api(owner).post(`/api/communities/${c.id}/groups`).send({ name: 'One too many' }),
        409,
        'limit_reached',
      );
      const g = await createGroup(owner, []);
      ...
      expectError(
        await api(owner)
          .post(`/api/groups/${g2}/members`)
          .send({ userIds: [extra!.id] }),
        409,
        'limit_reached',
      );
      const add = await api(owner)
        .post(`/api/communities/${c2.id}/members`)
        .send({ userIds: [extra!.id, extra2!.id] })
        .expect(200);
      expect(add.body).toMatchObject({
        added: [],
        failed: [
          { userId: extra!.id, reason: 'limit_reached' },
          { userId: extra2!.id, reason: 'limit_reached' },
        ],
      });
    });
  });
  ...
  describe('members', () => {
    it('GET members: owner/admins only, owner first', async () => {
      ...
      await api(owner)
        .post(`/api/communities/${c.id}/members`)
        .send({ userIds: [member!.id, admin!.id] })
        .expect(200);
      await api(owner)
        .put(`/api/communities/${c.id}/members/${admin!.id}/role`)
        .send({ role: 'admin' })
        .expect(204);
      const list = (await api(admin!).get(`/api/communities/${c.id}/members`).expect(200))
        .body as CommunityMember[];
    ...
    it('POST members: add rules (needsInvite, already_member, not_found) and the matrix events', async () => {
      ...
      const res = await api(owner)
        .post(`/api/communities/${c.id}/members`)
        .send({ userIds: [a!.id, shy!.id, existing!.id, unknown] })
        .expect(200);
      expect(res.body).toMatchObject({
        added: [a!.id],
        needsInvite: [shy!.id],
      ...
      expect(r[0]!.payload.chat).toMatchObject({
      ...
      expect(existingRec!.of('chat:updated')).toEqual([
    ...
    it('DELETE member: cascades out of every linked group and the announcement group; removed users cannot rejoin themselves', async () => {
      ...
      await api(owner)
        ...
        .expect(204);
      expectError(
        await api(other!).delete(`/api/communities/${c.id}/members/${victim!.id}`),
        403,
        'forbidden',
      );
      expectError(
        await api(admin!).delete(`/api/communities/${c.id}/members/${owner.id}`),
        403,
        'forbidden',
      );
      expectError(
        await api(admin!).delete(`/api/communities/${c.id}/members/${admin!.id}`),
        400,
        'validation_error',
      );
      ...
      const v = vRec!.log
        ...
        .map((e) => {
          if (e.event === 'message:new')
            return `new:${e.payload.message.chatId === g1 ? 'g1' : 'g2'}:${e.payload.message.system.kind}`;
          if (e.event === 'chat:upsert')
            return `upsert:${e.payload.chat.id === g1 ? 'g1' : 'g2'}:${e.payload.chat.membership}`;
          if (e.event === 'chat:removed')
            return `removed:${e.payload.chatId === c.announcementChatId ? 'ann' : '?'}`;
          return e.event;
        });
      expect(v).toEqual([
        'new:g1:member_removed',
        'upsert:g1:removed',
        'new:g2:member_removed',
        'upsert:g2:removed',
        'removed:ann',
        'community:removed',
      ]);
      ...
      expect(await memberRow(c.announcementChatId, victim!)).toMatchObject({
        hidden: true,
        leftReason: 'removed',
      });
      ...
      const communityCode = (await api(owner).get(`/api/communities/${c.id}/invite`).expect(200))
        .body.code;
      ...
      expect((await api(victim!).get(`/api/invites/${g3Code}`).expect(200)).body).toMatchObject({
        canJoin: false,
  ...
  describe('roles, ownership, leave', () => {
    it('roles mirror into the announcement group (admins can post); owner cannot be demoted', async () => {
      ...
      await api(owner)
        .post(`/api/communities/${c.id}/members`)
        .send({ userIds: [member!.id, member2!.id] })
        .expect(200);
      expectError(
        await api(member2!)
          .put(`/api/communities/${c.id}/members/${member!.id}/role`)
          .send({ role: 'admin' }),
        403,
        'forbidden',
      );
      ...
      await settle();
      const r = rec.log.filter((e) =>
        ['community:upsert', 'chat:upsert', 'chat:members-changed', 'message:new'].includes(
          e.event,
        ),
      );
      expect(r.map((e) => e.event)).toEqual([
      ...
      expect(r[1]!.payload.chat).toMatchObject({
        id: c.announcementChatId,
        myRole: 'admin',
        permissions: { canSend: true, canViewMembers: true },
      });
      expect((await memberRow(c.announcementChatId, member!)).role).toBe('admin');
      expectError(
        await api(member!)
          .put(`/api/communities/${c.id}/members/${owner.id}/role`)
          .send({ role: 'member' }),
        403,
        'forbidden',
      );
      ...
      await api(member!)
        .put(`/api/communities/${c.id}/members/${member2!.id}/role`)
        .send({ role: 'admin' })
        .expect(204);
      await api(owner)
        .put(`/api/communities/${c.id}/members/${member!.id}/role`)
      ...
      expectError(
        await api(owner)
          .put(`/api/communities/${c.id}/members/${crypto.randomUUID()}/role`)
          .send({ role: 'admin' }),
        404,
        'not_found',
      );
    ...
    it('transfer-ownership: owner only, target must be a member; mirrored into the announcement group', async () => {
      ...
      expectError(
        await api(member!)
          .post(`/api/communities/${c.id}/transfer-ownership`)
      ...
      await api(owner)
        ...
        .expect(204);
      await settle();
      expect(rec.names().filter((n) => ['community:upsert', 'chat:upsert'].includes(n))).toEqual([
        'community:upsert',
        'chat:upsert',
      ]);
      expect((await api(member!).get(`/api/communities/${c.id}`).expect(200)).body.myRole).toBe(
        'owner',
      );
      expect((await api(owner).get(`/api/communities/${c.id}`).expect(200)).body.myRole).toBe(
        'admin',
      );
    ...
    it('owner leaves: out of every group, succession to the oldest admin (mirrored), community:upsert + chat:upsert(ann) → new owner', async () => {
      ...
      await api(boss!)
        .post(`/api/communities/${c.id}/members`)
        .send({ userIds: [admin1!.id, admin2!.id] })
        .expect(200);
      await api(boss!)
        .put(`/api/communities/${c.id}/members/${admin2!.id}/role`)
        .send({ role: 'admin' })
        .expect(204);
      await api(boss!)
        .put(`/api/communities/${c.id}/members/${admin1!.id}/role`)
        .send({ role: 'admin' })
        .expect(204);
      await db
        ...
        .where(
          and(eq(communityMembers.communityId, c.id), eq(communityMembers.userId, admin1!.id)),
        );
      ...
      const b = bossRec!.log
        .filter((e) =>
          ['message:new', 'chat:upsert', 'chat:removed', 'community:removed'].includes(e.event),
        )
  ...
  describe('DELETE /communities/:id (deactivation)', () => {
    it('owner only; groups unlinked with a message; announcement group deleted; community:removed to every member', async () => {
      ...
      const m = memberRec!.log
        .filter((e) =>
          ['message:new', 'chat:updated', 'chat:removed', 'community:removed'].includes(e.event),
        )
        .map((e) => (e.event === 'message:new' ? e.payload.message.system.kind : e.event));
      expect(m).toEqual([
  ...
  describe('invite link', () => {
    it('owner/admins see and reset it; reset → community:upsert to owner/admins only', async () => {
      ...
      await api(owner)
        ...
        .send({ role: 'admin' })
        .expect(204);
      expect((await api(admin!).get(`/api/communities/${c.id}/invite`).expect(200)).body).toEqual({
        code: c.inviteCode,
      });
      expectError(await api(member!).get(`/api/communities/${c.id}/invite`), 403, 'forbidden');
      expectError(
        await api(member!).post(`/api/communities/${c.id}/invite/reset`),
        403,
        'forbidden',
      );
      ...
      await settle();
      expect(adminRec!.of('community:upsert').map((p) => p.community.inviteCode)).toEqual([
        body.code,
      ]);
