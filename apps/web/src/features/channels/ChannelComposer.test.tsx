 ...
 * chat media (postMedia.ts → enqueueMedia → uploadMedia); only the Document picker uploads
 * the original bytes.
 */
...
const processing = vi.hoisted(() => ({
  prepareVisualMedia: vi.fn(),
  isVisualMedia: (f: File) => f.type.startsWith('image/') || f.type.startsWith('video/'),
}));
...
describe('ChannelComposer uploads', () => {
  it('re-encodes a picked photo and uploads the clean copy with a thumbnail', async () => {
    ...
    expect(meta).toMatchObject({ kind: 'image', width: 1600, height: 1200 });
  ...
  it('never falls back to the original when the photo cannot be processed', async () => {
    ...
    // The photo is skipped with an error toast — never uploaded raw.
  ...
  it('uploads documents unchanged', async () => {
    uploads.uploadMedia.mockResolvedValue({ id: 'media-2' });
    const { container } = render(<ChannelComposer chat={chat} />);
    const doc = new File(['%PDF'], 'report.pdf', { type: 'application/pdf' });
    pick(container, null, doc);
    await vi.waitFor(() => expect(uploads.uploadMedia).toHaveBeenCalled());
    // The original bytes, as a file — no processing.
    expect(uploads.uploadMedia.mock.calls[0]![0]).toBe(doc);
    expect(uploads.uploadMedia.mock.calls[0]![1]).toMatchObject({ kind: 'file' });
