...
2. On each server `ready` (first connect **and** reconnects — the socket never replays) the
   domain resyncs run (calls first, in parallel with the chats: `call:rejoin` must beat the
   reconnect grace): clear typing indicators, reload `GET /api/chats` (a fresh request),
